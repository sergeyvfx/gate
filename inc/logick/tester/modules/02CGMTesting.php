<?php if ($__CGMTesting_Included__!='##CGMTesting_Included##') {$__CGMTesting_Included__='##CGMTesting_Included##';
  ...
  class CGMTestingATCheckbox extends CGMTestingATVirtual {
    ...
    function Draw ($s, $assoc_data, $task_num, $form_prefix='') {
      for ($i=0; $i<$s['anscount']; $i++) {
        $j=$assoc_data['ans'][$i];
        print ('<div class="ans"><input type="checkbox" class="cb" name="'.$form_prefix.'_task_'.$task_num.'_ans_'.$i.'" value="1">'.
  ...
  class CGMTestingATRadiobutton extends CGMTestingATVirtual {
    ...
    function Draw ($s, $assoc_data, $task_num, $form_prefix='') {
      for ($i=0; $i<$s['anscount']; $i++) {
        $j=$assoc_data['ans'][$i];
        print ('<div class="ans"><input type="radio" class="radio" name="'.$form_prefix.'_task_'.$task_num.'_ans" value="'.$i.'" group="1">'.
          '<span class="text">'.$s['ans'][$j]['text'].'</span></div>');
      }
    }
  ...
  class CGMTestingATString extends CGMTestingATVirtual {
    ...
    function Draw ($s, $assoc_data, $task_num, $form_prefix='') {
      print ('<div class="ans"><input type="text" class="block txt" name="'.$form_prefix.'_task_'.$task_num.'" value=""></div>');
    }
...
//
  ...
  class CGMTesting extends CGMVirtual {
    ...
    function InitIface () {
      ...
      if (($judge || $running || $manage) && ($this->Test_Obtained () || $this->Test_CanObtain ()))
        $this->gateway->AppendMainMenuItem ('Тестирование', '.?page=testing', 'testing');
    ...
    function DefaultContestSettings () { return array ('timestamp'=>time (), 'duration'=>0, 'trycount'=>0, 'groups'=>array (), 'judges'=>array (), 'autostart'=>false, 'timelimit'=>0); }
    function PerformCreation ($params) { $params=$this->DefaultContestSettings (); }
    ...
    function Contest_Manager ($id, $clear=false) {
      ...
      if ($clear) {
        global $act;
        if ($act=='showaddproblem')
          $this->InsertTemplate ('problem.form', array ('data'=>array (), 'lib'=>$this, 'act'=>'createproblem')); else
        if ($act=='editproblem') {
          global $uid;
    ...
    function Contest_ActionHandler () {
      ...
      if (!$this->GetAllowed ('CONTEST.MANAGE')) return;
      if ($act=='savegroups')    { $this->Contest_UpdateRecievedGroupUsed ($id); } else
      if ($act=='savecompilers') { $this->Contest_UpdateRecievedCompilers ($id); } else
      if ($act=='addcat')        { $this->Categories_Add ($id); } else
      if ($act=='delcat')        { $this->Categories_Del ($uid); } else
      if ($act=='upcat')         { $this->Categories_Up ($uid); } else
      if ($act=='downcat')       { $this->Categories_Down ($uid); } else
      if ($act=='createproblem') { $this->Problems_Create ($id, $catid); } else
      if ($act=='delproblem')    { $this->Problems_Delete ($uid); } else
      if ($act=='saveproblem')   { $this->Problems_Save ($uid); }
    } 
    ...
    function Categories_Add ($contest_id=-1) {
      ...
      if (trim ($name)=='') { add_info ('Имя категории не может быть пустым.'); return false; }
      if (db_count ('tester_categories', '`name`='.db_string ($name))) { add_info ('Категория с таким именем уже существует.'); return false; }
      $order=db_max ('tester_categories', 'order', '`contest_id`='.$WT_contest_id)+1;
    ...
    function Categories_Del ($id) {
      ...
      db_update ('tester_categories', array ('order'=>'`order`-1'), '(`contest_id`='.$r['contest_id'].') AND (`order`>'.$r['order'].')');
    ...
    function Problems_GetAtCat ($catid) {
      if (!$this->GetAllowed ('CONTEST.MANAGE')) return;
      $q=db_query ('SELECT `tp`.* FROM `tester_problems` AS `tp`, `tester_tasks` AS `tt` WHERE (`tp`.`id`=`tt`.`problem_id`) AND (`tt`.`catid`='.$catid.') ORDER BY `tp`.`name`');
      $arr=array ();
      while ($r=db_row ($q)) {
        $t=$r;
    ...
    function Problems_GetAll ($contest_id=-1) {
      ...
      $arr=array ();
      $q=db_query ('SELECT `tp`.*, `tt`.`catid` FROM `tester_problems` AS `tp`, `tester_tasks` AS `tt` WHERE '.
        '(`tp`.`id`=`tt`.`problem_id`) AND (`tt`.`contest_id`='.$contest_id.') ORDER BY `tt`.`catid`, `tp`.name');
    ...
    function Problem_Description ($num, $assoc_data, $form_prefix='') {
      return $this->Template ('problem.desc', array ('num'=>$num, 'assoc_data'=>$assoc_data, 'lib'=>$this, 'form_prefix'=>$form_prefix));
    }
    ...
    function PAGE_Testing () {
      ...
      $this->InsertTemplate ('testing', array ('data'=>$contest, 'lib'=>$this));
    }
    ...
    function PAGE_Status () {
      ...
      if (!WT_contest_running ($WT_contest_id) && !WT_contest_finished ($WT_contest_id) && !$manage) { content_unavaliable (); return; }
...
//        $this->Solution_DrawInformation ($id);
    ...
    function Test_Obtained ($contest_id=-1, $user_id=-1) {
      ...
      $s=$r['parameters'];
      if (db_affected ()>0 && !$s['finished'] && !$s['interrupted']) $res=true;
    ...
    function Test_CanObtain ($contest_id=-1, $user_id=-1) {
      ...
      if ($this->GetAllowed ('CONTEST.MANAGE')) return true;
      if ($this->IsContestJudge ($user_id)) return true;
      
      if (!WT_contest_running ($contest_id)) return false;
      ...
      if ($r['parameters']['started']) {
        ...
        if ($c['settings']['timelimit']) {
          ...
          if ($t+$c['settings']['timelimit']*60<time ()) {
            if (!$p['interrupted']) {
              $p['interrupted']=1;
              db_update ('tester_solutions', array ('parameters'=>db_string (serialize ($p)), 'points'=>0), '`id`='.$r['id']);
            }
            return false;
          }
        }
