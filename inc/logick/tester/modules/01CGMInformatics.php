<?php if ($__CGMInformatics_Included__!='##CGMInformatics_Included##') {$__CGMInformatics_Included__='##CGMInformatics_Included##';
  ...
  $INFORMATICS_ProblemSettingsFields=array (
    array ('name'=>'name',        'type'=>'TEXT',   'postname'=>'name',        'title'=>'Название задачи',              'important'=>true),
    array ('name'=>'timelimit',   'type'=>'TEXT',   'postname'=>'timelimit',   'title'=>'Ограничение по времени (сек)', 'important'=>true),
    array ('name'=>'memorylimit', 'type'=>'TEXT',   'postname'=>'memorylimit', 'title'=>'Ограничение по памяти (Мб)',   'important'=>true),
    array ('name'=>'input',       'type'=>'TEXT',   'postname'=>'input',       'title'=>'Входной файл',                 'important'=>true),
    array ('name'=>'output',      'type'=>'TEXT',   'postname'=>'output',      'title'=>'Выходной файл',                'important'=>true),
    array ('name'=>'tests',       'type'=>'CUSTOM', 'postname'=>'tests',       'title'=>'Разбалловка тестов',           'src'=>tpl_unparsed ('front/tester/modules/Informatics/problems.tests_automate'), 'important'=>true, 'check_value'=>'tests'),
    array ('name'=>'bonus',       'type'=>'TEXT',   'postname'=>'bonus',       'title'=>'Бонус',                        'important'=>true),
    array ('name'=>'archove',     'type'=>'CUSTOM', 'postname'=>'archive',     'title'=>'Архив с тестами',              'src'=>tpl_unparsed ('front/tester/modules/Informatics/problems.archive'), 'action'=>'save'),
    array ('name'=>'archove',     'type'=>'CUSTOM', 'postname'=>'archive',     'title'=>'Архив с тестами',              'src'=>tpl_unparsed ('front/tester/modules/Informatics/problems.archive'), 'action'=>'create', 'important'=>true, 'check_value'=>'archive'),
    array ('name'=>'checker',     'type'=>'CUSTOM', 'postname'=>'checker',     'title'=>'Готовый чекер',                'src'=>tpl_unparsed ('front/tester/modules/Informatics/problems.checker')),
    array ('name'=>'comment',     'type'=>'TEXT',   'postname'=>'comment',     'title'=>'Комментарий',                  'important'=>false),
  );

  class CGMRulesVirtual extends CVirtual {
    var $problem_container;
    function CGMRulesVirtual ($lib, $problem_container) { $this->lib=$lib;  $this->problem_container=$problem_container; }

    function GetStatusRow ($r, $last=false) { return ''; }
    function GetStatusHeader () { return ''; }

    function TaskStatus ($r) {
      if ($r['status']!=2) return 'Тестируется...';
      return WT_errors_string ($r['errors'], $r['parameters']['force_status']);
    }
    ...
    function SolutionsHeader () {
      return '<tr class="h"><th  class="first">Время</th><th width="25%">Участник</th><th width="25%">Задача</th><th width="60">Попытка</th><th width="25%">Результат</th>'.((!preg_match ('/ACM/', $this->GetClassName ()))?('<th width="60">Балл</th>'):('')).'<th class="last"><div style="background: transparent; width: 96px;">&nbsp;</div></th></tr>';
    }

    function SolutionsRow ($contest_id, $row_data, $n, $c, $perPage, $full, $acl) {
      ...
      $problem=$this->problem_container->GetProblemLetter (-1, $row_data['problem_id']) .'. <a href="?page=problems&act=view&id='.$row_data['problem_id'].'&redirect='.$redirect.'">'.htmlspecialchars ($pr['name']).'</a>';
      $status=$this->TaskStatus ($row_data);
      $points=$this->TaskPoints ($row_data);
      $try=$row_data['try'];

      $actions='';
      $actions.=stencil_ibtnav ('detail.gif' , $full.'&action=view&id='.$row_data['id'].'&redirect='.$redirect, 'Просмотреть решение');
      $actions.=stencil_ibtnav ('painter.gif', $full.'&action=rejudge&id='.$row_data['id'], 'Перетестировать решение');
      $actions.=stencil_ibtnav ('mem.gif',     $full.'&action=setml&id='.$row_data['id'], 'Установить &laquo;Превышение памяти&raquo;');
      if ($acl['delete']) $actions.=stencil_ibtnav ('cross.gif', $full.'&action=delete&id='.$row_data['id'], 'Удалить', 'Удалить это решение?');

      $time=$this->lib->GetSolutionTime ($row_data);

      return '<tr'.(($class!='')?(' class="'.$class.'"'):('')).'><td>'.$time.'</td><td>'.$user.'</td><td>'.$problem.'</td><td align="center">'.$try.'</td><td>'.$status.'</td>'.((!preg_match ('/ACM/', $this->GetClassName ()))?('<td align="center">'.$points.'</td>'):('')).'<td align="right">'.$actions.'</td></tr>';
    }
  ...
  class CGMRulesACM extends CGMRulesVirtual {
    ...
    function CGMRulesACM ($lib, $problem_container) { CGMRulesVirtual::CGMRulesVirtual ($lib, $problem_container); $this->SetClassName ('CGMRulesACM'); }

    function TaskStatus ($r) {
      if ($r['status']!=2) return 'Тестируется...';
      return WT_errors_string ($r['parameters']['TESTS'], $r['parameters']['force_status'], true, $r['errors']);
    }

    function GetStatusHeader () { return '<tr class="h"><th width="40%" class="first">Задача</th><th width="60">Попытка</th><th>Результат</th><th class="last">&nbsp;</th></tr>'; }
    ...
    function GetStatusRow ($contest_id, $r, $n, $c, $perPage, $full='', $acl=array ()) {
      ...
      $class='';
      if ($c==$perPage-1 || $c==$n-1) $class='last';
      if ($r['status']==2 && $r['errors']=='OK') $class.=(($class!='')?(' '):('')).'subactive';
      $actions=stencil_ibtnav ('detail.gif' , $full.'&action=view&id='.$r['id'].'&redirect='.$redirect, 'Просмотреть решение');
      return '<tr'.(($class!='')?(' class="'.$class.'"'):('')).'><td>'.$title.'</td><td align="center">'.$try.'</td><td>'.$status.'</td><td align="right" width="48">'.$actions.'</td></tr>';
    }

    function Monitor ($contest_id=-1) {
      global $WT_contest_id, $CORE;
      if ($contest_id<0) $contest_id=$WT_contest_id;
      $this->lib->InsertTemplate ('monitor.acm', array ('lib'=>$this->lib, 'contest_id'=>$contest_id, 'user_id'=>user_id ()));
    }
  }

  class CGMRulesKirov extends CGMRulesVirtual {
    function CGMRulesKirov ($lib, $problem_container) { CGMRulesVirtual::CGMRulesVirtual ($lib, $problem_container); $this->SetClassName ('CGMRulesKirov'); }

    function GetStatusHeader () { return '<tr class="h"><th width="40%" class="first">Задача</th><th width="60">Попытка</th><th width="40%">Результат</th><th>Балл</th><th class="last">&nbsp;</th></tr>'; }
    ...
    function Monitor ($contest_id=-1) {
      global $WT_contest_id, $CORE;
      if ($contest_id<0) $contest_id=$WT_contest_id;
      $this->lib->InsertTemplate ('monitor.kirov', array ('lib'=>$this->lib, 'contest_id'=>$contest_id, 'user_id'=>user_id ()));
    }
  }

  class CGMInformaticsProblemsContainer {
    var $data=array ();
    var $problem_setts=array ('timelimit', 'memorylimit', 'input', 'output', 'tests', 'bonus', 'comment', 'checker');
    var $cache=array ();
    ...
    function FillData () {
      $this->data=array ();
      $q=db_select ('tester_problems', array ('id', 'lid', 'settings', 'name', 'uploaded'), '`lid`=0', 'ORDER BY `lid`, `name`');
      while ($r=db_row ($q)) {
        $arr=$r;
    ...
    function ProblemDescription ($id) {
      if (isset ($this->cache['Problem.Description'][$id])) return $this->cache['Problem.Description'][$id];
    ...
    function GetByField ($f, $v) {
      $n=count ($this->data);
      for ($i=0; $i<$n; $i++)
        if ($this->data[$i][$f]==$v) return $this->data[$i];
      return $arr;
    }

    function GetByName ($name,$skipid=-1) {
      $tmp=$this->GetByField ('name', $name);
      if ($tmp['id']==$skipid) return array ();
      return $tmp;
    }
    ...
    function Create ($name, $settings) {
      ...
      db_insert ('tester_problems', array ('name'=>db_html_string ($name), 'description'=>db_string ($settings['desc']),
        'settings'=>db_string ($s), 'lid'=>0));

      $id=db_last_insert ();
      $arr['filename']=$this->StoreArchive ($id, $_FILES['TestsArchive']);

      db_update ('tester_problems', array ('settings'=>db_string (serialize ($arr))), "`id`=$id");
    ...
    function CreateReceived () {
      if ($this->Create (FormPOSTValue ('name', 'ProblemSettings'), INFORMATICS_GenerateProblemEditorForm ())) {
        if (FormPOSTValue ('addToContest', 'ProblemSettings')) {
          global $promtadd;
          $this->lib->Contenst_AddProblemToContest ($promtadd, db_last_insert ());
        }
        $_POST=array ();
      }
    }
    ...
    function Update ($id, $settings) {
      ...
      $desc=$settings['desc'];
      if (trim ($name)=='') { add_info ('Название задачи не может быть пустым.'); return false; }
      if ($this->GetByName (htmlspecialchars ($name), $id)) { add_info ('Задача с таким именем уже существует.'); return false; }

      $a=$this->GetById ($id);

      $arr=$a['settings']; $n=count ($this->problem_setts);
      for ($i=0; $i<$n; $i++)
        $arr[$this->problem_setts[$i]]=$settings[$this->problem_setts[$i]];
      ...
      $s=serialize ($arr);
      db_update ('tester_problems', array ('name'=>db_html_string ($name), 'description'=>db_string ($desc),
        'settings'=>db_string ($s), 'uploaded'=>$uploaded), "`id`=$id");
    ...
    function GetProblemLetter ($contest_id, $problem_id) {
      ...
      if ($contest_id<0) $contest_id=$WT_contest_id;
      if (isset ($this->cache['letters'][$contest_id][$problem_id])) return $this->cache['letters'][$contest_id][$problem_id];
      $this->UpdateTasksCache ($contest_id, $problem_id);
      $arr=$this->cache['tasks'][$contest_id]; $n=count ($arr);
      for ($i=0; $i<$n; $i++)
        if ($arr[$i]['problem_id']==$problem_id) $res=core_alpha ($arr[$i]['letter']);
  ...
  class CGMInformatics extends CGMVirtual {
    ...
    function RegisterRules ($class, $title, $id) { $this->rules[]=array ('class'=>$class, 'title'=>$title, 'id'=>$id); }
    ...
    function DefaultContestSettings () { return array ('rules'=>0, 'timestamp'=>time (), 'duration'=>0, 'trycount'=>0, 'penalty'=>0, 'compilers'=>array (),
      'groups'=>array (), 'judges'=>array (), 'freezetime'=>0, 'unfrozen'=>false, 'viewdetail'=>false, 'show_last_accepted'=>false, 'ignore_ce'=>false, 'autostart'=>false, 'archive'=>''); }
    ...
    function PerformContestStateUpdate ($c, $state) {
      ...
      if ($c['settings']['archive']!='') {
       if ($c['settings']['archive.blocked']!=$blocked) {
          ...
          db_update ('tester_contests', array ('settings'=>db_string (serialize ($c['settings']))), '`id`='.$c['id']);
        }
      }
    }
    ...
    function IPC_Problem_DescriptionForm ($id, $cid, $backlink='') {
      if ($id>=0 && $this->Problem_Accessible ($id, $cid))
        return $this->Problem_DescriptionForm ($id, $backlink); else {
    ...
    function GetProblemsCountAtContest ($contest_id) { return db_count ('tester_tasks', "`contest_id`=$contest_id"); }
    ...
    function ProblemLetter ($contest_id, $problem_id) {
      return db_field_value ('tester_tasks', 'letter', "`contest_id`=$contest_id AND `problem_id`=$problem_id");
    }
    ...
    function PutSolution ($contest_id, $user_id, $problem_id, $compiler_id, $src) {
      $params=array ('src'=>$src, 'compiler_id'=>$compiler_id);
      db_insert ('tester_solutions', array ('lid'=>0, 'contest_id'=>$contest_id, 'problem_id'=>$problem_id,
        'user_id'=>$user_id, 'timestamp'=>time (), 'status'=>0, 'parameters'=>db_string (serialize ($params)), 'errors'=>'""', 'points'=>0));
      return true;
    }
    ...
    function Solution_SetML ($solution_id) {
      ...
      db_update ('tester_solutions', array ('errors'=>'"ML"', 'points'=>0, 'status'=>2, 'parameters'=>db_string (serialize ($params))), "`id`=$solution_id");
    }
    ...
    function Solution_Rejudge ($solution_id) {
      ...
      db_update ('tester_solutions', array ('errors'=>'""', 'points'=>0, 'status'=>0, 'parameters'=>db_string (serialize ($params))), "`id`=$solution_id");
    }
    ...
    function Solution_DrawInformation ($solution_id) {
      ...
      if (db_affected ()<=0) return;
      $r['parameters']=unserialize ($r['parameters']);
      $detail=false;
      if (!$allow) {
        if ($r['user_id']!=user_id ()) return;
        $c=WT_contest_by_id ($r['contest_id']);
        if ($c['settings']['viewdetail']) $detail=true;
      } else $detail=true;
    ...
    function Contest_ActionHandler () {
      ...
      if ($act=='view') {
        if (browser_engine ()!='DONKEY')
          $this->CPrintLn ('<div id="view" style="margin-top: -6px; padding-top: 6px;">'); else
    ...
    function Contest_ToggleDisableProblem ($id, $pid) {
      if (!$this->GetAllowed ('CONTEST.MANAGE')) return false;
      if (db_count ('tester_disabled_problems', "(`contest_id`=$id) AND (`problem_id`=$pid)")>0)
        db_delete ('tester_disabled_problems', "(`contest_id`=$id) AND (`problem_id`=$pid)"); else
    ...
    function Contest_UpdateRecievedGroupUsed_Iterator ($id=-1, $name, $table) {
      ...
      for ($i=0; $i<$n; $i++) {
        $gid=$glist[$i];
        if (db_count ($table, "`contest_id`=$id AND `group_id`=$gid")>0) continue;
    ...
    function Contest_UpdateRecievedGroupUsed ($id=-1) {
      if (!$this->GetAllowed ('CONTEST.MANAGE')) return;
      $this->Contest_UpdateRecievedGroupUsed_Iterator ($id, 'usergroup',  'tester_contestgroup');
      $this->Contest_UpdateRecievedGroupUsed_Iterator ($id, 'judgegroup', 'tester_judgegroup');
    }
    ...
    function Contenst_AddProblemToContest ($contest_id, $problem_id) {
      if (!$this->GetAllowed ('CONTEST.MANAGE')) return;
      if (db_count ('tester_tasks', "`contest_id`=$contest_id AND `problem_id`=$problem_id")>0) return false;
    ...
    function Contest_Save ($id, $clear=false) {
      ...
      if (trim ($name)=='') { add_info ('Название контеста не может быть пустым.'); return false; }
    ...
    function Problem_IsAtContest ($problem_id, $contest_id) {
      return (db_count ('tester_tasks', "`contest_id`=$contest_id AND `problem_id`=$problem_id")>0);
    }
    ...
    function Problem_AccessibleForSubmit ($problem_id, $user_id, $silent=true, $contest_id=-1) {
      ...
      $contest=WT_contest_by_id ($contest_id);
      $add='';
      if ($contest['settings']['ignore_ce']) $add=' AND (`errors`<>"CE") ';
    ...
    function Problem_DescriptionForm ($id, $backlink='') {
      ...
      $res=$this->Template ('problems.description', array ('data'=>$arr, 'backlink'=>$backlink, 'editbtn'=>$edit));
      return $res;
    }

    function Problem_DrawDescription ($id, $backlink='') { $this->CPrintLn ($this->Problem_DescriptionForm ($id, $backlink)); }
    ...
    function Problem_GenerateEditorForm ($data=array (), $act='create', $backlink='') {
      ...
      $action=prepare_arg ('.?page=prbmanager&action='.$act.(($id!='')?('&id='.$id):('')).'&'.(($pageid!='')?('pageid='.$pageid):('')).
        (($promtadd!='')?('&promtadd='.$promtadd):('')).(($backlink!='')?('&redirect='.urlencode ($backlink)):('')));
      $desc=$data['desc'];
      if ($desc=='') { $tpl=manage_template_by_name ('Олимпиады / Informatics / Заготовка условия задачи'); $desc=$tpl->GetText (); }
    ...
    function Problem_GetDataFromPostData () { return INFORMATICS_GenerateProblemEditorForm (); }
    ...
    function Problem_Accepted ($id) {
      global $WT_contest_id;
      $user_id=user_id ();
      return db_count ('tester_solutions', "(`contest_id`=$WT_contest_id) AND (`user_id`=$user_id) AND (`problem_id`=$id) AND (`errors`='OK') AND (`status`=2)");
    }

    function Problem_Tried ($id) {
      global $WT_contest_id;
      $user_id=user_id ();
      return db_count ('tester_solutions', "(`contest_id`=$WT_contest_id) AND (`user_id`=$user_id) AND (`problem_id`=$id) AND (`status`=2)");
    }
    ...
    function GetUserSolutionsCount ($contest_id=-1, $user_id) { return $this->GetSolutionsCountEntry ($contest_id, "`user_id`=$user_id"); }
    function GetAllSolutionsCount  ($contest_id=-1)           { return $this->GetSolutionsCountEntry ($contest_id); }
    ...
    function PAGE_Problems () {
      ...
      if ($this->GetAllowed ('CONTEST.MANAGE'))
        $this->AppendQuickLink ('Редактировать контест', '?page=contest&action=manage&id='.$WT_contest_id);

      if ($this->GetAllowed ('PROBLEMS.CREATE'))
        $this->AppendQuickLink ('Создать задачу', '?page=prbmanager&action=showcreate&promtadd=1&redirect='.get_redirection ());
    }
    ...
    function PAGE_Monitor () {
      ...
      $this->AppendQuickLink ('В отдельном окне', 'JavaScript:monitor2fullscreen();');
    }
    ...
    function PAGE_DrawCreateProblemForm ($backlink='') {
      if (!$this->GetAllowed ('PROBLEMS.CREATE')) return;
      $f=$this->Problem_GenerateEditorForm ($this->Problem_GetDataFromPostData (), 'create', $backlink);
    ...
    function PAGE_DrawEditProblemForm ($id, $backlink='', $tried=false) {
      if (!$this->GetAllowed ('PROBLEMS.CREATE')) return;
      if (!$tried)
        $arr=$this->Problem_GetDataById ($id); else
    ...
    function PAGE_ProblemsManager_ProblemsHandler () {
      ...
      if (!$this->GetAllowed ('PROBLEMS.MANAGE')) return;
      ...
      if ($edit) {
        ...
        if ($action=='edit') {
          $this->gateway->AppendNavigation ('Редактирование задачи', '.?page=prbmanager&action=edit&id='.$id);
      ...
      if ($create) {
        if ($action=='showcreate') {
          $this->gateway->AppendNavigation ('Создание задачи', '.?page=prbmanager&action=showcreate');
      ...
      if ($create) {
        $this->CPrintLn ('<script language="JavaScript" type="text/javascript">var descInited=false; function InitDesc () { if (descInited) return; iframeEditor_Init (\'desc\'); descInited=true; };</script>');
    ...
    function PAGE_ProblemsManager_CheckersHandler () {
      if (!$this->GetAllowed ('CHECKERS.MANAGE')) return;
      $this->InsertTemplate ('checkers.list', array ('lib'=>$this));
      $this->gateway->AppendNavigation ('Управление чекерами', '?page=prbmanager&action=checkers'.(($pageid!='')?('&pageid='.$pageid):('')));
    }
    ...
    function PAGE_ProblemsManager () {
      ...
      if (!$this->GetAllowed ('CHECKERS.MANAGE') && $action=='checkers') $action='';
    ...
    function InitIface () {
      ...
      //if ($this->GetAllSolutionsCount ()>0)
      ...
      if ($started || $finished || $manage)  {
        if ($c>0) $this->gateway->AppendMainMenuItem ('Список задач',   '.?page=problems' , 'problems');
      }
    ...
    function IsContestJudge ($id=-1) {
      global $WT_contest_id;
      if ($id<0) $id=$WT_contest_id;
      if (isset ($this->cache[$id]['IsContestJudge'])) return $this->cache[$id]['IsContestJudge'];
      $this->cache[$id]['IsContestJudge']=$this->GetAllowed ('CONTEST.MANAGE');
      if ($this->cache[$id]['IsContestJudge']) return true;

      if ($id=='') return;

      $q=db_query ('SELECT COUNT(*) AS `c` FROM `usergroup` AS `ug`, `tester_judgegroup` AS `tjg` '.
                ' WHERE (`ug`.`user_id`='.user_id ().') AND (`tjg`.`group_id`=`ug`.`group_id`) AND (`tjg`.`contest_id`='.$id.')');
      $r=db_row ($q);
      $res=$r['c']>0;
    ...
    function Subnav_Info () {
      ...
      return $this->Template ('subnav_info', array ('data'=>$data, 'lib'=>$this));
    }
    ...
    function GetLastAcceptedAtContest ($id=1) {
      ...
      $user_clause='`user_id` IN (SELECT `u`.`id` FROM `user` AS `u`, `usergroup` AS `ug`, `tester_contestgroup` AS `tcg` WHERE '.
        '(`u`.`id`=`ug`.`user_id`) AND (`ug`.`group_id`=`tcg`.`group_id`) AND (`tcg`.`contest_id`='.$id.') )';

      $skip_user_clause='NOT (`user_id` IN (SELECT `u`.`id` FROM `user` AS `u`, `tester_judgegroup` AS `tjg`, `usergroup` AS `ug` WHERE '.
        '(`u`.`id`=`ug`.`user_id`) AND (`ug`.`group_id`=`tjg`.`group_id`) AND (`tjg`.`contest_id`='.$id.')))';

      $arr=db_row_value ('tester_solutions', " ($user_clause) AND $skip_user_clause AND (`contest_id`=$id) AND (`status`=2) AND (`errors`=\"OK\")".
        (($timestamp>0)?(" AND (`timestamp`<=$timestamp)"):(''))."", "ORDER BY `timestamp` DESC");
