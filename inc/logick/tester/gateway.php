<?php if ($_gateway_included_!='##gateway_Included##') {$_gateway_included_='##gateway_Included##';
  global $WT_gateway;

  $WT_main_menu=array (
    array ('Список контестов', '?page=contest', 'contest', 'Page_Contest',    ACCESS_GUEST),
  );
  ...
  class CGateway extends CVirtual {
    ...
    function InitMenus () {
      ...
      for ($i=0; $i<$n; $i++)
        $this->AppendMainMenuItem ($WT_main_menu[$i][0], $WT_main_menu[$i][1], $WT_main_menu[$i][2], $WT_main_menu[$i][4]);
    }
    ...
    function SpawnContestContainer () { return WT_spawn_new_contest_container (); }
    function SpawnLibraryContainer () { return WT_spawn_new_library_container (); }

    ////////
    //

    function AppendMainMenuItem ($title, $url, $tag, $access=0) {
      if ($access!='' && user_access ()<$access) return;
      $this->main_menu->AppendItem ($title, $url, $tag);
    }
    function  AppendLIBHandler ($lid, $name, $handler) { $this->LIB_Handlers[$lid][$name]=$handler; }

    function AppendNavigation   ($title, $url, $hint='') { $this->nav[]=array ('title'=>$title, 'url'=>$url, 'hint'=>$hint); }
    function PrependNavigation  ($title, $url, $hint='') { $t=$this->nav; $this->nav=array (); $this->AppendNavigation ($title, $url, $hint); for ($i=0, $n=count ($t); $i<$n; $i++) $this->nav[]=$t[$i]; }
    function CPrint             ($text)               { $this->content.=$text; }
    function CPrintLn           ($text)               { $this->content.=$text."\n"; }
    function GetContent         ()                    { return $this->content; }
    
    function AppendQuickLink   ($cpt, $url)          { $this->qlinks[]=array ('cpt'=>$cpt, 'url'=>$url); }
    
    ////////
    //

    function GetCurrentContest () { return $this->current_contest; }

    ////////
    // Outputing stuff

    function InsertTemplate ($name, $args=array ()) { $this->CPrintLn (tpl ('front/tester/'.$name, $args)); }

    function DrawNavigator () {

      if (!isset ($this->current_contest['id']))
        $this->PrependNavigation ('Олимпиады', '/'); else
        $this->PrependNavigation ($this->current_contest['name'], '/?action=manage&id='.$this->current_contest['id'], 'Управление контестом');

      $root=config_get ('document-root').'/tester';
      $n=count ($this->nav);
      print ('<div id="snavigator">');
      for ($i=0; $i<$n-1; $i++)
        print ('<a href="'.$root.$this->nav[$i]['url'].'"'.(($this->nav[$i]['hint']!='')?('title="'.$this->nav[$i]['hint'].'"'):('')).'>'.$this->nav[$i]['title'].'</a>');
    ...
    function DrawManage () {
      global $action;
      if (!$this->AdminMode ()) { println ('HACKERS?'); return; }
      if ($action=='save') {
    ...
    function Draw () {
      ...
      if ($this->current_contest['lid']!='') {
        ...
        if ($lib!=nil) $lib->InitIface ();
      }
      if (wiki_admin_page () && $this->AdminMode ()) { $this->DrawManage (); print $this->content; return; }
      ...
      if (count ($this->qlinks)>0) {
        ...
        for ($i=0; $i<count ($this->qlinks); $i++)
          println ('  '.(($i>0)?('::'):('')).'<a href="'.$this->qlinks[$i]['url'].'">'.$this->qlinks[$i]['cpt'].'</a>');
        println ('</div>');
      }
    }
    ...
    ////////
    //
    function GetAllowed ($action) { return $this->security->GetAllowed ($action); }
    ...
    function Handle () {
      ...
      if (wiki_admin_page ()) return false;
      // Local handlers
      $handler=''; $n=count ($WT_main_menu);
      for ($i=0; $i<$n; $i++)
    ...
    function Page_Contest () {
      ...
      if (!isset ($clear)) {
        if (isset ($changeto))              { $this->SwitchToContest ($changeto); if ($manage) redirect ('.?page=contest&action=manage&id='.$changeto); }
        if ($action=='create'  && $create)  { $ccnt->CreateReceived (); redirect ('SELF', array ('action'=>'')); }
        if ($action=='delete'  && $del)     { $ccnt->Delete ($id); redirect ('SELF', array ('action'=>'', 'id'=>'')); }
        if ($manage) {
          if ($action=='stop' || $action=='start' || $action=='restart' )  { $ccnt->$action ($id); }
          if ($action=='save'    && $manage)  { $cnt=WT_spawn_new_contest_container (0); $action='manage'; $cnt->Save ($id); /*if ($cnt->Save ($id)) redirect ('.?page=contest&action=manage&id='.$id);*/ }
        }
        if ($action=='manage'  && !$manage) { unset ($action); unset ($id); }


        if ($pageManage) $this->CPrintLn ('<table width="100%"><tr valign="top"><td width="50%" style="padding-right: 2px;">');
        $this->CPrintLn (stencil_formo ('title=Список доступных контестов;'.((!$this->GetAllowed ('CONTEST.CREATE'))?('smb=true;'):(''))));
        $this->InsertTemplate ('contest.list', array ('data'=>$ccnt->GetAccessibleList (user_id ()),
          'current_contest'=>$this->current_contest, 'accManage'=>$manage, 'accDel'=>$del));
        $this->CPrintLn (stencil_formc ());
        if ($pageManage) {
        ...
        if ($this->GetAllowed ('CONTEST.CREATE')) $this->Page_DrawContentCreate  ();
      } else {
        if ($pageManage)
    ...
    function Handle_SaveSecurity () { 
      ...
      db_update ('tester', array ('security'=>db_string ($this->security->SerializeData ())));
    }
  ...
  function WT_spawn_new_gateway () {
    global $WT_gateway;
    if ($WT_gateway!=nil) return $WT_gateway;
