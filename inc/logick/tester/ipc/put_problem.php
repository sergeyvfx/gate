<?php if ($_WT_ipc_put_problem_included_!='###WT_IPC_PutProblem_Inclided###') { $_WT_ipc_put_problem_included_='###WT_IPC_PutProblem_Inclided###';

  function WT_PutProblem () {
    global $id, $lid, $err, $desc;

    if (!WT_IPC_CheckLogin ()) return;
    if ($id=='')  { print ('Void filename for WT_PutProblem()'); return; }
    if ($lid=='') { print ('Void library identifier for WT_PutProblem()'); return; }
