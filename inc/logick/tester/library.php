<?php if ($_WT_library_included_!='##WT_library_Included##') {$_WT_library_included_!='##WT_library_Included##';
  ...
  class CGLibraryContainer {
    ...
    function Register ($class, $pseudo, $lid) { $this->data[]=array ('class'=>$class, 'pseudonym'=>$pseudo, 'lid'=>$lid); }
    function GetList () { return $this->data; }
  }

  function WT_spawn_new_library_container () {
    global $WT_library_container;
    if ($WT_library_container!=nil) return $WT_library_container;
  ...
  function WT_spawn_new_library ($lid,$gw=nil) {
    global $WT_libraries;
    if (isset ($WT_libraries[$lid])) return $WT_libraries[$lid];
    if ($gw==nil) $gw=WT_spawn_new_gateway ();
  ...
  function WT_receive_library_from_selector ($name='') { return $_POST[$name.'_lib_selector']; }
}
?>
