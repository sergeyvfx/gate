<?php if ($_CCPublication_!='#CCPublication_Included#') {$_CCPublication_='#CCPublication_included#';
  class CCPublication extends CCVirtual {
    var $data, $ids, $uids, $dbids, $dbuids,$idlink;
    var $scripts=array (
      array ('script'=>'data',    'file'=>'data.php',    'desc'=>'Скрипт просмотра статьи'),
      array ('script'=>'edit',    'file'=>'edit.php',    'desc'=>'Скрипт редактироваия статьи')
    );
    ...
    function UpdateScripts () {
      if ($this->settings['detailed']) {
        $this->scripts[]=array ('script'=>'text/data',    'file'=>'text/data.php');
        $this->scripts[]=array ('script'=>'text/edit',    'file'=>'text/edit.php');
        $this->scripts[]=array ('script'=>'text/history', 'file'=>'text/history.php');
        $this->scripts[]=array ('script'=>'text/index',   'file'=>'text/index.php');
      }
    }
    ...
    function InitInstance () {
      if (CCVirtual::InitInstance ()) return;
      if ($this->settings['dataset']['id']=='') $this->settings['dataset']['id']=-1;
      $this->dataset=$this->SpawnDataSet ();
      $this->UpdateScripts ();
    }

    function DrawSettingsForm ($formname='') {
      println ('<input type="checkbox" name="'.$formname.'_detailed" value="1"> Использовать &laquo;подробные&raquo; публикации<div id="hr"></div>');
    ...
    function ReceiveSettings ($formname='') {
      ...
      if ($this->dataset==null) return false;
      if ($this->dataset->GetID ()<0) { add_info ('Не указан набор данных'); return false; }
      $this->dataset->Ref ();
      $fields=array ('uid' => 'INT', 'order'=>'INT');
      $arr=$this->dataset->GenCreateFields ();
      foreach ($arr as $k=>$v) $fields[$k]=$v;
      ...
      manage_settings_create ('Количество элементов на странице для раздела '.
        '«'.htmlspecialchars ($this->GetName ()).'»', 'Разделы', $s, 'CSCSignedNumber');
      opt_set ($s, '10');
    ...
    function ReceiveContentByQuery ($clause='') {
      $this->idlink=$this->dbuids=$this->uids=$this->dbids=$this->ids=$this->data=array ();
      if ($this->settings['content']=='') return;
      $q=db_select ($this->settings['content'], array ('*'), $clause, 'ORDER BY '.(($this->IsDated ())?(' `date`,'):('')).' `order`, `timestamp` ');
      $arr=array (); $ids=array (); $uids=array ();
      $usage=array ();
      while ($r=db_row ($q)) {
        if (isset ($usage[$r['uid']]))
          if ($arr[$usage[$r['uid']]]['timestamp']<$r['timestamp'])
            $strip[$usage[$r['uid']]]=true; else $strip[count ($arr)+1]=true;
        $usage[$r['uid']]=count ($arr);
        $arr[]=$r;
      }
      $save=$arr; $n=count ($arr); $arr=array ();
      for ($i=0; $i<$n; $i++)
        if (!isset ($strip[$i])) $arr[]=$save[$i];
      $arr[]=array ('uid'=>'-1'); $n=count ($arr); $luid=-1;
      for ($i=0; $i<$n; $i++) {
        if ($luid==-1) $luid=$arr[$i]['uid'];
        if ($arr[$i]['uid']!=$luid) {
    ...
    function Editor_Save ($formname='', $receive=true) {
      global $redirect;
      if (!$this->GetAllowed ('ADDINFO')) return;
      if ($receive) $this->dataset->ReceiveData ($formname);
      $arr=$this->dataset->GetFieldValues (true);
      $clause=''; if ($this->IsDated ()) $clause='`date`="'.$this->dataset->FieldValue ('date').'"';
      $arr['order']=db_next_order ($this->settings['content'], $clause);
      $arr['timestamp']=time ();
      $arr['user_id']='"'.user_id ().'"';
      $arr['ip']='"'.get_real_ip ().'"';
      db_insert ($this->settings['content'], $arr);
      $uid=db_last_insert (); db_update ($this->settings['content'], array ('uid'=>$uid), '`id`='.$uid);
    ...
    function DrawPublistEntry ($formname, $titleField, $data) {
      ...
      for ($i=0; $i<$n; $i++) {
        ...
        if (trim ($title)=='') $title='<i>Нет значения</i>';
        $title='<a href="'.$url.'&uid='.$data[$i]['uid'].'">'.$title.'</a>';
        print ('  <tr'.(($i==$n-1)?(' class="last"'):('')).'><td class="n">'.($i+1).'.</td><td>'.$title.'</td>');
        $actions='';
        if ($edit)
        ...
        if ($actions=='') $actions='&nbsp;';
        print ('<td align="right">'.$actions.'</td>');
        println ('</tr>');
      }
      $this->dataset->FreeValues ();
      println ('</table>');
    ...
    ////////
    function IsDated () { $fields=$this->dataset->Fields (); $f=$fields[0]->GetDataClass (); return ($f=='CDCDate'); }
    function GetCurDate () { global $year, $month, $day; return "$year-$month-$day"; }
    
    function GetYears () {
      $res=array (); $usage=array (); $n=count ($this->data);
      for ($i=0; $i<$n; $i++) { $y=preg_replace ('/([0-9]+)\-([0-9]+)\-([0-9]+)/si', '\1', $this->data[$i]['date']); if (!$usage[$y]) {$res[]=array ('title'=>$y, 'val'=>$y); $usage[$y]=true;} }
//      array_multisort ($res, SORT_ASC, SORT_STRING);
    ...
    function GetDays ($year, $month) {
      $res=array (); $usage=array (); $n=count ($this->data);
      for ($i=0; $i<$n; $i++) {
        $y=preg_replace ('/([0-9]+)\-([0-9]+)\-([0-9]+)/si', '\1', $this->data[$i]['date']);
        $m=preg_replace ('/([0-9]+)\-([0-9]+)\-([0-9]+)/si', '\2', $this->data[$i]['date']);
        $d=preg_replace ('/([0-9]+)\-([0-9]+)\-([0-9]+)/si', '\3', $this->data[$i]['date']);
        if (!$usage[$d] && $y==$year && $m==$month) {$res[]=array ('title'=>$d, 'val'=>$d); $usage[$d]=true; }
      }
//      array_multisort ($res, SORT_ASC, SORT_STRING);
      return $res;
    }
...
//      array_multisort ($res, SORT_ASC, SORT_STRING);
    ...
    function DrawDatedHead ($titleField='') {
      ...
      if (trim ($t)=='') $t='<i>Пустое поле</i>';
      // navigator
      if ($year!='') {
        $sub='<b>'.$year.'</b> год.';
        print ('<div id="snavigator"><a href="'.$url.'">Выбор года</a>');
        if ($month!='') {
          $sub='<b>'.$months[$month].'</b>, '.$sub;
          print ('<a href="'.$url.'&year='.$year.'">Выбор месяца</a>');
          if ($day=='')
            println ('Выберите число</div>'); else {
              $sub='<b>'.$day.'</b>, '.$sub;
              print ('<a href="'.$url.'&year='.$year.'&month='.$month.'">Выбор числа</a>');
              if ($uid=='')
                println ('Список публикаций</div>'); else {
                  print ('<a href="'.$url.'&year='.$year.'&month='.$month.'&day='.$day.'">Список публикаций</a>');
                  if ($action!='edit')
                    println ($t.'</div>'); else
                    println ('<a href="'.$url.'&year='.$year.'&month='.$month.'&day='.$day.'&uid='.$uid.'">'.$t.'</a>Редактирование</div>');
                }
            }
        } else
          println ('Выберите месяц</div>');
      } else println ('<div id="snavigator">Выберите год</a></div>');
      // Subinfo
      if ($sub!='' && $uid=='') println ('<div class="contentSub">Публикации за '.$sub.'</div>');
    }
    ...
    function DrawDatedIterator ($title, $arr,$url_prefix) {
      ...
      println ('<table class="list smb">');
      for ($i=0; $i<$n; $i++) {
        println ('<tr'.(($i==$n-1)?(' class="last"'):('')).'><td class="n">'.($i+1).'.</td><td><a href="'.$url_prefix.$arr[$i]['val'].'">'.$arr[$i]['title'].'</a></td></tr>');
      }
      println ('</table>');
    }

    function DrawYears ()       { $arr=$this->GetYears (); content_url_var_push_global ('function'); $url=content_url_get_full ().'&year='; $this->DrawDatedIterator ('год', $arr,$url); }
    function DrawMonths ($year) { $arr=$this->GetMonths ($year); content_url_var_push_global ('function'); $url=content_url_get_full ().'&year='.$year.'&month='; $this->DrawDatedIterator ('месяц', $arr,$url); }
    function DrawDays   ($year,$month) { $arr=$this->GetDays ($year,$month); content_url_var_push_global ('function'); $url=content_url_get_full ().'&year='.$year.'&month='.$month.'&day='; $this->DrawDatedIterator ('день', $arr,$url); }

    function GetDataWithDate ($y, $m, $d) {
      $d="$y-$m-$d";
      $arr=array (); $n=count ($this->data);
      for ($i=0; $i<$n; $i++) {
        if ($this->data[$i]['date']==$d) $arr[]=$this->data[$i];
      }
      return $arr;
    }
    ...
    function DrawDatedPublist ($formname, $titleField) {
      ...
      if ($uid!='' && $this->data[$this->uids[$uid]]['date']!="$year-$month-$day") $uid='';
      if ($day!='' && !count ($this->GetDataWithDate ($year, $month, $day))) {$day=''; $uid='';}
      if ($year=='')
        $this->DrawYears (); else
      if ($month=='')
        $this->DrawMonths ($year); else
      if ($day=='')
        $this->DrawDays ($year, $month); else {
          ...
          if ($uid=='') {
            ...
            if ($action!='edit') {
              println ('<div class="contentSub"><span class="arr"><a href="'.content_url_get_full ().'&uid='.$uid.'&action=edit">Редактировать</a> последнюю версию</span></div>');
    ...
    function DrawSimplePublist ($formname, $titleField) {
      ...
      if ($uid!='') {
        ...
        print ('<div id="snavigator"><a href="'.$url.'">Список публикаций</a>');
        if ($action!='edit') {
          println ($t.'</div>');
          println ('<div class="contentSub"><a href="'.content_url_get_full ().'&uid='.$uid.'&action=edit">Редактировать</a> последнюю версию</div>');
          $this->DrawPublicationHistory ($uid, $formname);
        } else {
          println ('<a href="'.content_url_get_full ().'&uid='.$uid.'">'.$t.'</a>Редактирование</div>');
          if ($this->GetAllowed ('EDITINFO'))
            $this->DrawPublicationEditor ($uid, $formname, $id);
        }
      } else {
    ...
    function DrawPublicationHistory ($uid, $formname='',$args='',$head=true) {
      ...
      $url=content_url_get_full ();
      $one=db_affected ()<=1;
      if ($args=='') $args='action=edit&id';
      while ($r=db_row ($q)) {
        $time='<a href="'.$url.'&'.$args.'='.$r['id'].'">'.format_ltime ($r['timestamp']).'</a>';
        $user=user_generate_info_string ($r['user_id']);
        $actions='';
        if ($editinfo) $actions.='[<a href="'.content_url_get_full ().'&action=rollback&id='.$r['id'].'">Вернуться к этой версии</a>]';
        if ($delinfo && !($one && !$del)) $actions.=stencil_ibtnav ('minus_s.gif', content_url_get_full ().'&action=delete&id='.$r['id'], 'Удалить', 'Удалить эту версию публикации?');
    ...
    function DrawNavigation ($formname='') {
      ...
      $f=$fields[0]->GetField ();
      if ($f!='date')
        $this->DrawSimplePublist ($formname, $f); else
        $this->DrawDatedPublist ($formname, $fields[1]->GetField ());
    }

    function DrawAddPublication ($formname='') { $this->dataset->DrawEditorForm ($formname, content_url_get_full ()); }

    function DeletePublication ($id, $update=false) {
      if (!$this->GetAllowed ('DELETEINFO')) return;
      $r=db_row (db_select ($this->settings['content'], array ('*'), '`id`='.$id));
      if (db_affected ()<=1 && !$this->GetAllowed ('DELETE')) return;
      $this->dataset->SetFieldValues ($r);
      $this->dataset->FreeContent ();
      db_delete ($this->settings['content'], '`id`='.$id);
      if ($update) $this->ReceiveContent ();
    }

    function DeleteEntirePublication ($uid) {
      if (!$this->GetAllowed ('DELETE')) return;
      $q=db_select ($this->settings['content'], array ('id'), '`uid`='.$uid);
      while ($r=db_row ($q))
    ...
    function Move ($dir,$uid) {
      ...
      $clause='';
      if ($this->IsDated ()) $clause='`date`="'.$this->GetCurDate ().'"';
      if ($dir=='down') db_move_down ($this->settings['content'], $uid, $clause, 'uid');
      if ($dir=='up')   db_move_up   ($this->settings['content'], $uid, $clause, 'uid');
      $this->ReceiveContent ();
    }
...
//      if (!$this->GetAllowed ('EDIT')) return;
    ...
    function Editor_EditForm ($formname='') {
      ...
      if ($f!='') $this->$f ($formname);
    }

    function Editor_ItemEditForm ($formname='') {
      global $uid;
      if (!isset ($this->uids[$uid])) redirect ('..');
      $this->Editor_ExecAction ($formname);
    ...
    function Editor_DrawHistory ($formname='') {
      ...
      $this->Editor_ExecAction ($formname);
      if (!isset ($this->uids[$uid])) redirect ('..');
      if ($action=='view') redirect ('.?uid='.$uid.'&oldid='.$id);
      $this->DrawPublicationHistory ($uid, $formname, 'action=view&id', false);
    }
    ...
    function GetListRow ($i) {
      ...
      $this->dataset->FreeValues ();
      foreach ($r as $k=>$v) $arr[$k]=$v;
      return $arr;
    }
    function GetListRowByUid ($uid) { return $this->GetListRow ($this->uids[$uid]); }
    function GetListRowById ($id) {
      $res=db_row_value ($this->settings['content'], '`id`='.$id);
      $this->dataset->SetFieldValues ($res);
      $r=$this->dataset->GetFieldValues (false, true);
      $this->dataset->FreeValues ();
      foreach ($r as $k=>$v) $res[$k]=$v;
      return $res;
    }
    function GetList () {
      $arr=array ();
      $n=$this->GetListLength ();
      for ($i=0; $i<$n; $i++) $arr[]=$this->GetListRow ($i);
      return $arr;
    }
    ...
    function GetPagintation ($page=0, $pageid='page') {
      ...
      return stencil_pagintation ($pageCount, $page, content_url_get_full (), $pageid);
    }
    ...
    function GetItemEditorLink ($id) {
      ...
      if ($this->IsDated ()) {
        $y=preg_replace ('/([0-9]+)\-([0-9]+)\-([0-9]+)/si', '\1', $data['date']);
        $m=preg_replace ('/([0-9]+)\-([0-9]+)\-([0-9]+)/si', '\2', $data['date']);
        $d=preg_replace ('/([0-9]+)\-([0-9]+)\-([0-9]+)/si', '\3', $data['date']);
        return $url.'&wiki=edit&action=edit&year='.$y.'&month='.$m.'&day='.$d.'&uid='.$data['uid'].'&redirect='.$redirect;
      } else
        return $url.'&wiki=edit&action=edit&uid='.$data['uid'].'&redirect='.$redirect;
    ...
    function DisplayScript () {
      ...
      $c=manage_spawn_template ($this->settings['script']); return $c->GetText ();
    }
    function ItemScript    () { $c=manage_spawn_template ($this->settings['itemScript']); return $c->GetText (); }
