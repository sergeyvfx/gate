<?php if ($_CCList_!='#CCList_Included#') {$_CCList_='#CCList_included#';
  class CCList extends CCVirtual {
    ...
    function DrawSettingsForm () { manage_draw_dataset_selector_for_content ($this->GetClassName ()); }
    ...
    function ReceiveContentByQuery ($clause='', $suff='') {
      if (isset ($this->settings['content']))
        $r=db_row (db_select ($this->settings['content'], array ('*'), $clause, $suff)); else
        $r=array ();
    ...
    function ReceiveLastContent   (){ $this->ReceiveContentByQuery ('', 'ORDER BY `timestamp` DESC  LIMIT 1'); }
    function ReceiveContentWithTime ($timestamp) { $this->ReceiveContentByQuery ("`timestamp`=$timestamp"); }
    function ReceiveContentWithId   ($id)        { if ($id=='') $id=-1; $this->ReceiveContentByQuery ("`id`=$id"); }
    function ReceiveContent   (){ $this->ReceiveLastContent (); }
    ...
    function Rollback ($commit_id) {
      if (!$this->GetAllowed ('EDIT')) return;
      if ($commit_id=='') return;
      if ($commit_id==$this->ReceiveLastContentId ()) return;
      // TODO: Add reorting stuff here
      if ($commit_id==$this->content_id) return;
    ...
    function Editor_DrawHistory () {
      ...
      if ($action=='delete') {
        ...
        if ($del) $this->DeleteContentById ($id);
      } else
      if ($action=='rollback') {
        redirector_add_skipvar ('action', 'rollback');
        redirector_add_skipvar ('id');
        if ($edit) $this->Rollback ($id);
      }
      ...
      if (db_affected ()>0) {
        ...
        while ($r=db_row ($q)) {
          ...
          $actions='';
          if ($edit) $actions.='[<a href=".?wiki=history&action=rollback&id='.$r['id'].'">Вернуться к этой версии</a>]';
          if ($del) $actions.=stencil_ibtnav('minus_s.gif', content_url_get_full().'&action=delete&id='.$r['id'], 'Удалить', 'Удалить эту версию статьи?');
          if ($actions!='') $actions=' | '.$actions;
          if ($i<2)
            println ('  <li><div class="top">'.$time.' | '.$user.' '.$actions.'</div></li>'); else
    ...
    function Field ($field) { return $this->dataset->FieldValue ($field); }

    function Editor_DrawContent  ($vars=array ()) {
      global $pIFACE, $oldid;
      $pIFACE=$this;
      if ($oldid!='') $this->ReceiveContentWithId ($oldid);
