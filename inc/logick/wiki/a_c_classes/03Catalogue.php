<?php if ($_CCCatalogue_Included_!='##CCCatalogue_Included##') { $_CCCatalogue_Included_='##CCCatalogue_Included##';
  class CCCatalogue extends CCVirtual {
    ...
    function GetCatalogueData ($depth, $pid) {
      ...
      $tmp=arr_from_ret_query (db_select ($table, array ('*'), $clause, 'ORDER BY `order` ASC, `timestamp` DESC'));
      ...
      for ($i=0; $i<$n; $i++) {
        $it=$tmp[$i];
        if (isset ($uids_usage[$it['uid']])) continue;
        $uids_usage[$it['uid']]=1;
    ...
    function GetCatalogueItem ($depth, $uid, $id=-1, $preparse=true) {
      ...
      $cat_id = $this->GetCatIDByDepth ($depth);
      if ($id<0)
        $arr=db_row_value ($table, "`uid`=$uid", 'ORDER BY `id` DESC'); else
        $arr=db_row_value ($table, "`id`=$id");
      if ($preparse) $arr=$this->ParseDataRow ($cat_id, $arr);
    ...
    function ReceiveSettings ($formname='') {
      $this->settings['content']=content_create_support_table ($this->content_id, -1, array ('name'=>'TEXT', 'dataset'=>'INT'));
    ...
    function CatNameById ($id) { return db_field_value ($this->settings['content'], 'name', "`id`=$id"); }

    function GetNextUID ($id, $pid) {
      $table=content_support_table_name ($this->content_id, $this->GetCatDatasetID ($id), $id);
    ...
    function MaxDepth () {
      if (isset ($this->cache['MaxDepth'])) return $this->cache['MaxDepth'];
      $this->cache['MaxDepth']=db_count ($this->settings['content']);
      return $this->cache['MaxDepth'];
    }

    function GetNextCatID ($id) {
      if (isset ($this->cache['NextCatID'][$id])) return $this->cache['NextCatID'][$id];
    ...
    function GetCatItemOrder ($cat_id, $uid) { return $this->GetCatItemField ($cat_id, $uid, 'order'); }

    function GetCatDatasetID ($id) {
      $dataset_id=-1;
      if (isset ($this->cache['Catalogue.DatasetID'][$id]))
        return $this->cache['Catalogue.DatasetID'][$id]; else
        $dataset_id=$this->cache['Catalogue.DatasetID'][$id]=db_field_value ($this->settings['content'], 'dataset', "`id`=$id");
      return $dataset_id;
    }

    function GetCatDataset ($cat_id) {
      if (isset ($this->cache['Catalogue.Dataset'][$cat_id])) return $this->cache['Catalogue.Dataset'][$cat_id];
    ...
    function GetCatIDByDepth ($depth) {
      if ($this->cache['Catalogue.Depth'][$depth]) return $this->cache['Catalogue.Depth'][$depth];
      $id=db_field_value ($this->settings['content'], 'id', '', 'ORDER BY `id` LIMIT '.$depth.',1');
      $this->cache['Catalogue.Depth'][$depth]=$id;
      return $id;
    }

    function GetSupportTableByDepth ($depth) {
      if ($this->cache['SupportTable.Depth'][$depth]) return $this->cache['SupportTable.Depth'][$depth];
    ...
    function GetSupportTableByCatID ($id) {
      if ($this->cache['SupportTable.ID'][$id]) return $this->cache['SupportTable.ID'][$id];
      $table=content_support_table_name ($this->content_id, $this->GetCatDatasetID ($id), $id);
    ...
    function GetCatDepthByID ($cat_id) {
      if (isset ($this->cache['CatDepthByID'][$cat_id])) return $this->cache['CatDepthByID'][$cat_id];
    ...
    function AppendCatFromDataset ($name, $dataset) {
      ...
      if (trim ($name)=='') { add_info ('Название подкаталога не может быть пустым.'); return false; }
      $fields=array ('pid'=>'INT', 'uid'=>'INT', 'order'=>'INT');
      $arr=$dataset->GenCreateFields ();
      foreach ($arr as $k=>$v) $fields[$k]=$v;

      db_insert ($this->settings['content'], array ('name'=>db_html_string ($name), 'dataset'=>$dataset->GetID (),
        'timestamp'=>time (), 'user_id'=>user_id (), 'ip'=>db_html_string (get_real_ip ())));
    ...
    function DeleteCatRecursive_Iterator ($content_id, $dataset, $uid) {
      $q=db_select (content_support_table_name ($content_id, $dataset->GetID (), $uid));
      while ($r=db_row ($q)) {
    ...
    function AppendDataToCatFromDataset ($cat_id, $pid, $dataset) {
      ...
      $arr=$this->BuildDBArrayFromDataset ($dataset, $pid, $this->GetNextUID ($cat_id, $pid), $this->GetNextUID ($cat_id, $pid));
      $table=$this->GetSupportTableByCatID ($cat_id);
      db_insert ($table, $arr);
    }
    ...
    function CheckCatItemExistment ($cat_id, $uid, $dataset) {
      ...
      $table=$this->GetSupportTableByCatID ($cat_id);
      return db_count ($table, "`id`=$id".((trim ($query)!='')?(" AND $query"):('')))>0;
    }

    function UpdateCatItemFromDataset ($cat_id, $uid, $dataset) {
      if (!$this->GetAllowed ('EDITINFO')) return;
      if ($this->CheckCatItemExistment ($cat_id, $uid, $dataset)) return;
    ...
    function MoveItemDown ($cat_id, $uid) {
      if (!$this->GetAllowed ('EDIT')) return;
      $pid=$this->GetCatItemParent ($cat_id, $uid);
      db_move_down ($this->GetSupportTableByCatID ($cat_id), $uid, "`pid`=$pid", 'uid');
    }
    ...
    function GetCatItemHistory ($cat_id, $uid, $preparse=true) {
      ...
      if ($preparse) $arr=$this->ParseDataArr ($cat_id, $arr);
    ...
    function DrawCatItemHistory ($cat_id, $uid) {
      ...
      for ($i=0; $i<$n; $i++) {
        ...
        if ($editinfo) $actions.='[<a href="'.$full.'&action=rollback&iid='.$r['id'].'">Вернуться к этой версии</a>]';
        if ($delinfo)  $actions.=stencil_ibtnav ('minus_s.gif', $full.'&action=delete&iid='.$r['id'], 'Удалить', 'Удалить этот эдемент?');
    ...
    function Editor_ActionHandler () {
      global $act, $pid;
      if ($act=='create') $this->AppendReceivedCat ();   else
      if ($act=='delete') $this->DeleteCatRecursive ($pid); else
      if ($act=='save')   { if (!$this->UpdateRecievedCat ($pid)) $act='edit'; }
      if ($act=='edit')   $this->Editor_EditCat ($pid);
    }
    ...
    function Editor_DrawCurrentStructure () {
      ...
      while ($r=db_row ($q)) {
        ...
        println ('<table class="list" width="100%" style="margin: 2px 0 2px '.($interion*24).'px;"><tr class="h"><th class="first">'.$r['name'].'</th><th width="80" style="text-align: right;" class="last">'.$actions.'</th></tr></table>');
        $interion++;
      }
      formc ();
    }
    ...
    function Editor_CheckScript () {
      println ('<script language="JavaScript" type="text/javascript">function check (frm, id) { if (qtrim (getElementById (id).value)=="") { alert ("Название подкаталога не может быть пустым."); return false; } frm.submit (); } </script>');
    }

    function Editor_EditCat ($id) {
      $full=content_url_get_full ();
      formo ('title=Редактирование каталога;');
      settings_formo ($full.'&act=save&pid='.$id, 'POST', 'onsubmit="check (this, \'edname\'); return false;"');
      $value=$this->CatNameById ($id);
      if ($_POST['edname']!='') $value=htmlspecialchars (stripslashes ($_POST['edname']));
      println ('Название:<input type="text" class="txt block" name="edname" id="edname" value="'.$value.'">');
      settings_formc ($full);
      formc ();
    }
    ...
    function Editor_Structure () {
      ...
      settings_formo (content_url_get_full ().'&act=create', 'POST', 'onsubmit="check (this, \'name\'); return false;"');
      println ('Название:<input type="text" class="txt block" name="name" id="name"><div id="hr"></div>');
    ...
    function Editor_EditForm_ActionHandler ($formname='') {
      ...
      if ($action=='delete') {
        if ($iid=='')
          $this->DeleteFromCat ($cid, $id); else
          $this->DeleteCatItem ($cid, $iid);
      }
      if ($action=='up')       $this->MoveItemUp    ($cid, $id);
      if ($action=='down')     $this->MoveItemDown  ($cid, $id);
      if ($action=='rollback') $this->RollbackToCatItemID ($cid, $id, $iid);
    }

    function Editor_EditForm_CurCatId () { $depth=$this->Editor_EditForm_CurCatDepth (); return $this->GetCatIDByDepth ($depth); }

    function Editor_EditForm_CurCatPID () {
      global $ids;
      $depth=$this->Editor_EditForm_CurCatDepth ();
      $arr=explode (',', $ids);
      if (isset ($arr[$depth-1])) return $arr[$depth-1];
      return -1;
    }
    ...
    function Editor_EditForm_DrawNavigator ($formname='') {
      ...
      for ($i=0; $i<=$depth; $i++) {
        ...
        if ($i<$depth || $action=='edit') $title='<a href=".?wiki=edit'.(($newIDS!='')?('&ids='.$newIDS):('')).'">'.$title.'</a>';
        $newIDS.=(($mewIDS!='')?(','):('')).$ids[$i];
        print $title;
      }
      ...
      if ($action=='edit') {
        if ($iid!='')
          print '<a href=".?wiki=edit'.(($newIDS!='')?('&ids='.$newIDS):('')).'&action=edit&id='.$id.'">История</a>';
    ...
    function Editor_EditForm_DrawItems ($formname='') {
      ...
      for ($i=0; $i<$n; $i++) {
        ...
        $actions='';
        if ($editinfo) $actions.=stencil_ibtnav ('edit.gif', $full.'&action=edit&id='.$it['uid']);
        if ($edit)     $actions.=stencil_updownbtn ($i, $n, $it['uid'], $full);
        if ($del)      $actions.=stencil_ibtnav ('cross.gif', $full.'&action=delete&id='.$it['uid'], 'Удалить', 'Удалить этот элемент и все го вложения?');
    ...
    function Editor_Scripts () {
      ...
      print 'Скрипт отображения статьи:';
