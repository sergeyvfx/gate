<?php if ($_CCVirtual_!='#CCVirtual_Included#') {$_CCVirtual_='#CCVirtual_included#';
  class CCVirtual extends CVirtual {
    ...
    function Init ($content_id=-1, $security=nil)  {
      ...
      $this->content_id=$content_id; $this->security=$security; $this->InitInstance ($content_id);
    }
    function InitInstance     ()     { if ($this->instanceInitialized) return false; $this->instanceInitialized=true; return true; }
    function DrawSettingsForm ($formname='') { print ('<span class="shade">Настройки данного класса отсутствуют.</span>'); }
    ...
    function Editor_CreateDirContent ($d) {
      ...
      for ($i=0; $i<count ($this->scripts); $i++) {
        $s=$this->scripts[$i];
        $mk=dirname ($s['file']);
        if ($mk!='' && $mk!='.' && !file_exists ($d.'/'.$mk)) { mkdir ($d.'/'.$mk);  chmod ($d.'/'.$mk, 0775); }
        $src=get_file ($tplDir.'/'.$s['script']);
        if (preg_match ('/index\.php$/', $s['file'])) $src=preg_replace ('/\$\{up_to_root\}/', $up, $src);
    ...
    function OverwriteDisplayScript ($val) { $this->force_displayScript=$val; }

    function TemplatesDir () { return tpl_dir ().'/back/cclasses/'.$this->GetCLassName (); }
...
//  content_Register_CClass ('CCVirtual', 'CCVirtual');
}
?>
