<?php if ($_wiki_included_!='#wiki_Included#') {$_wiki_included_='#wiki_Included#'; 
  ...
  function wiki_initialize () {
    if (config_get ('check-database')) {
      if (!db_table_exists ('content')) {
        ...
        db_insert ('content', array ('order'=>'1', 'pid'=>'0', 'name'=>'"Корневой раздел"', 'path'=>'"/"',
          'settings'=>'"'.addslashes ('a:1:{s:8:"security";a:1:{s:3:"ALL";a:2:{s:5:"order";s:10:"allow_deny";s:4:"acts";a:1:{i:0;a:2:{s:3:"act";s:8:"AllowAll";s:3:"val";s:0:"";}}}}}').'"'));
      }
    }
  }
  ...
  function wiki_get_indexfile ($action) {
    ...
    if ($action=='edit')    $file='edit.php';
    if ($action=='history') $file='history.php';
    return $file;
  }
  ...
  function wiki_get_page ($url) {
    ...
    if ($error) { return $src; return; }
    $cur=prepare_arg (get_redirection (false, true));
    $items=array ();
    $uidurl=(($uid!='')?('uid\='.$uid):(''));
    $dir=$url;
    if ($url[strlen ($url)-1]=='/') $url.='?'; else $url.='&';
    if ($uidurl!='') $uidurl='&'.$uidurl;
