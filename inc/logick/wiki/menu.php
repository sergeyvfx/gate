<?php if ($_wiki_NavMenu_included_!='##wiki_NavMenu_Included##') {$_wiki_NavMenu_included_!='##wiki_NavMenu_Included##';
  $wiki_menu_rules=array (
    array ('pattern'=>'/\[url.*href\s*=\s*&quot;(.*)&quot;.*\](.*)\[\/url\]/si', 'replace'=>'<a href="\1">\2</a>')
  );
  ...
  function wiki_menu_parse_item ($src, $params) {
    ...
    if ($wiki_menu_global_vars==nil) {
      $wiki_menu_global_vars=array (
        'anon_redirect' => ((!user_authorized ())?('redirect='.get_redirection ()):('')),
  ...
  function wiki_items_src ($items, $indent=0) {
    ...
    for ($i=0; $i<$n; $i++) {
      ...
      if (!$nomarker && !$opened) { $res.="$prefix<ul>\n"; $opened=true; }
      if ($nomarker && $opened)   { $res.="$prefix</ul>\n"; $opened=false; }
      $res.="$prefix  ".(($nomarker)?(''):('<li>')).$items[$i]['src'];
      if (count ($items[$i]['items'])>0) $res.="\n".wiki_items_src ($items[$i]['items'], $indent+2).$prefix.'  ';
      $res.=(($nomarker)?(''):("</li>\n"));
    }
    if ($opened) $res.="$prefix</ul>\n";
    return $res;
  }

  function wiki_menu_parse ($src) {
    $menus=array (); $menu_uk=-1;
    $src=preg_replace ('/\<\!--.*--\>/si', '', $src);
    $res=''; $n=strlen ($src);
    $curDepth=0; $menuItems=array ();
    while ($i<$n) {
      // Get da string
      $buf='';
      while ($src[$i]<=' ' && $i<$n) $i++;
      while ($src[$i]!="\n" && $src[$i]!="\r" && $i<$n)  { $buf.=$src[$i]; $i++; }
      ...
      if ($buf[0]=='.' || $buf[0]==',') {
        ...
        if ($depth>$curDepth) {
          $menuItems[$curDepth+1]=&$menuItems[$curDepth][count ($menuItems[$curDepth])-1]['items'];
          $curDepth++;
        } else $curDepth=$depth;
        $item=preg_replace ('/([\.\,]+\s*)(.*)/si', '\2', $buf);
        $s=wiki_menu_parse_item ($item, &$params);
        if ($s!=false)
          $menuItems[$curDepth][]=array ('src'=>$s, 'nomarker'=>$buf[0]==',');
      }
      $i++;
    }
    ...
    $n=count ($menus);
    for ($i=0; $i<$n; $i++) {
      $res.="<div class=\"menu\"".(($i>0)?(' style="padding-top: 8px;"'):('')).">\n";
