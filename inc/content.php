<?php if ($_content_included_!='#content_Included#') {$_content_included_='#content_Included#'; 
  ...
  function content_RegisterClass ($group, $className, $classPseudonym='') {
    ...
    $content_RegisteredClasses[$group][]=array ('class'=>$className, 'pseudonym'=>$classPseudonym);
  }
  ...
  function content_support_table_name ($id, $dataset_id, $uid=-1) { return 'content_support_'.$id.'_'.$dataset_id.(($uid!=-1)?("_$uid"):('')); }

  function content_create_support_table ($id, $dataset_id, $fields=array (), $uid=-1) {
    $tname='content_support_'.$id.'_'.$dataset_id.(($uid!=-1)?("_$uid"):(''));
    if (db_count ('content_support_tables', "`content_id`=$id AND `dataset_id`=$dataset_id AND `unique`=$uid")>0) return $tname;
  ...
  function content_destroy_support_table ($id, $dataset_id, $uid=-1) {
    if (db_count ('content_support_tables', "`content_id`=$id AND `dataset_id`=$dataset_id AND `unique`=$uid")==0) return;
    $tname='content_support_'.$id.'_'.$dataset_id.(($uid!=-1)?("_$uid"):(''));
  ...
  // Sttaic page's content
  function content_static_page ($fname) {
    $fname=config_get ('site-root').$fname.'/'.config_get ('data-file');
    if (($src=get_file ($fname))==false) $src=content_error_page ('404', array ('url'=>$fname));
  ...
  function content_get_allowed ($action) {
    ...
    if ($content_security) return $content_security->GetAllowed ($action);
    return false;
  }
  
  function content_security_set () { global $content_security; return $content_security!=null; }
  ...
  function content_recursive_move ($src, $dst) {
    ...
    if (!file_exists ($dst)) { mkdir ($dst); chmod ($dst, 0775); }
