<?php if ($_service_Included_!='#_service_Included') { $_service_Included_='#_service_Included';
  ...
  class CService extends CVirtual {
    ...
    function Create () {
      ...
      $this->SpawnService ();
      $this->service->Create ();
      if (!$this->service->CanCreate ()) return false;
      ...
      db_insert ('service', array ('name'=>"\"$name\"", 'sclass'=>"\"$class\"", 'settings'=>"\"$settings\""));
      return true;
    }
    ...
    function DrawSettingsForm () { if ($this->service) $this->service->DrawSettingsForm (); }
    ...
    function GetServiceName () { if ($this->service) return $this->service->GetServiceName (); }
  ...
  function service_by_id ($id) { global $services_link; return $services_link[$id]; }
  ...
  function manage_service_create () { $c=manage_spawn_service (-1, $_POST['service']); $c->CreateReceived (); service_append ($c); }
  function manage_service_update_received ($id) { $c=service_by_id ($id); $c->UpdateReceived (); }
  function manage_service_delete ($id) { $c=service_by_id ($id); if ($c) { $c->Destroy (); service_unset_by_id ($id); } }
  function manage_service_get_list () { return arr_from_query ('SELECT * FROM `service` ORDER BY `name`'); }
}
?>
