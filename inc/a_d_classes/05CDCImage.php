<?php if ($_CDCImage_!='#CDCImage_included#') {$_CDCImage_='#CDCImage_included#';
  ...
  class CDCImage extends CDCVirtual {
    ...
    function UpdateLimitSetting ($arr, $title, $field, $suff) {
      if ($_POST['cntset_'.$field.'_'.$suff])
        if (!isnumber ($_POST['cntset_'.$field.'_'.$suff.'_val'])) {
          ...
          return false;
        } else $arr[$suff]=$_POST['cntset_'.$field.'_'.$suff].$_POST['cntset_'.$field.'_'.$suff.'_val'];
      return true;
    }
    ...
    function IncludeStuff () {
      ...
      if ($image_stuff_included) return;
      print ('
        <script language="JavaScript" type="text/javascript">
          function CDCImage_OnImageUpload (field, formname, url, val, w, h, mime) {
            if (val=="") { url=document_root+"/pics/clear.gif"; hide (formname+"_"+field+"_infoBox"); } else {
