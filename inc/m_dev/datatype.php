<?php if ($_manage_dt_included_!='#manage_dt_Included#') {$_manage_dt_included_='#manage_dt_Included#'; 
  ...
  class CDataType extends CVirtual {
    ...
    function Create () {
      ...
      if (db_count ('datatypes', '`name`="'.$name.'"')<=0) {
        db_insert ('datatypes', array ('name'=>"\"$name\"", 'class'=>'"'.addslashes ($this->data->GetClassName ()).'"',
    ...
    function Destroy () {
      if (manage_datatype_refcount ($this->id)>0) return;
    ...
    function DrawEditorForm     ($field, $formname='', $init=true) { $this->data->DrawEditorForm ($field, $formname, $init); }
    function ReceiveValue       ($field, $formname='') { $this->data->ReceiveValue ($field, $formname); }

    function BuildQueryValue    ()               { return $this->data->BuildQueryValue (); }
    function BuildCheckImportancy  ($field, $formname='') { return $this->data->BuildCheckImportancy ($field, $formname); }
    function GetDBFieldType     ()   { return $this->data->GetDBFieldType (); }

    function BuildInitScript       ($field, $formname='') { return $this->data->BuildInitScript ($field, $formname); }

    function DrawContentSettingsForm ($title, $field) { return $this->data->DrawContentSettingsForm ($title, $field); }
    function ReceiveContentSettings  ($title, $field) {
      if (!$this->data->ReceiveContentSettings ($title, $field))
        return false;
      $this->settings['data']=$this->data->SerializeSettings ();
//      db_update ('datatypes', array ('settings'=>db_string ($this->SerializeSettings ())), '`id`='.$this->id);
      return true;
    }

    function NewContentSpawned      ($field, $content_id=-1) { return $this->data->NewContentSpawned ($field, $content_id); }
    function PerformContentDeletion ($field, $content_id)    { return $this->data->PerformContentDeletion ($field, $content_id); }
    
    function SetValue ($v) { $this->data->SetValue ($v); }
    function GetValue ()   { return $this->data->GetValue (); }
    function Value ()      { return $this->data->Value (); }
    function GetDataSettings    ()   { $a=$this->data->GetSettings (); return $a['data']; }
    function UpdateDataSettings ($v) { return $this->data->UpdateSettings ($v); }

    function FreeContent () { $this->data->DestroyValue (); }
    function FreeValue ()   { $this->data->FreeValue (); }
  }

  function manage_spawn_datatype ($id, $name='', $baseClass='', $settings=array ()) {
    $c=new CDataType ();
    $c->Init ($id);
    if ($id<0) {
      $c->SetName ($name);
  ...
  function manage_datatype_refcount ($id)     { return db_field_value ('datatypes', 'refcount', "`id`=$id"); }
  ...
  function manage_datatype_create          ($name, $baseClass, $settings) { $c=manage_spawn_datatype (-1, $name, $baseClass, $settings); return $c->Create (); }
  function manage_datatype_received_create ()                             { $c=manage_spawn_datatype (-1); return $c->CreateReceived (); }

  function manage_datatype_delete ($id) { $c=manage_spawn_datatype ($id); $c->Destroy (); }
  function manage_datatype_update_received ($id) { $c=manage_spawn_datatype ($id); $c->UpdateReceived (); }

  /* TOTO: !!! PATCJH ME !!! */
  function manage_datatype_getlist () {
    $q=db_query ('SELECT * FROM `datatypes` ORDER BY `name`');
    $arr=array ();
    while ($r=db_row ($q)) $arr[]=$r;
    return $arr;
  }
  ...
  function manage_datatype_get_by_name ($name) {
    ...
    for ($i=0; $i<count ($classes); $i++)
      if ($classes[$i]['class']==$name) return $classes[$i];
    return array ();
  }
}
?>
