<?php if ($_manage_tpl_included_!='#manage_tpl_Included#') {$_manage_tpl_included_='#manage_tpl_Included#'; 
  ...
  class CTemplate extends CVirtual {
    ...
    function Init ($id=-1, $name='', $text='') {
      ...
      $this->name=$name; $this->text=$text; $this->refcount=0;
      if ($id!=-1 && $name=='') {
...
//        $r=array ();
        if (isset ($manage_template_cache[$id]['text']))
          $r=$manage_template_cache[$id]; else
          {
    ...
    function Ref () { if ($this->GetID ()<0) return; db_update ('templates', array ('refcount'=>'`refcount`+1'), '`id`='.$this->GetID ()); $this->refcount++; }

    function Check ($silent=false) {
      if (trim ($this->GetName ())=='') { if (!$silent) add_info ('Имя шаблона не может быть пустым.'); return false; }
      if (manage_template_exists ($this->name, $this->id)) { if (!$silent) add_info ('Шаблон с таким именем уже существует.'); return false; }
      return true;
    }
    ...
    function Update () {
      manage_template_cache_update_item ($this->id, $this->GetName (), $this->GetText ());
    ...
    function Draw () { print eval ('?>'.$this->GetText ()); }
  }

  function manage_spawn_template ($id=-1, $name='', $text='') { $c=new CTemplate ($id, $name, $text); return $c; }
  ...
  function manage_template_exists ($__name, $ignore=-1) {
    ...
    if ($manage_template_cache==nil) {
      ...
      while ($r=db_row ($q)) $manage_template_cache[$r['id']]=$r;
    }
    // Search in cache
  ...
  // Cache updators
  function manage_unregister_template ($id)        { global $manage_template_cache; unset ($manage_template_cache[$id]); }
  function manage_template_cache_update_item ($id, $name, $text) { global $manage_template_cache; $manage_template_cache[$id]['name']=$name; $manage_template_cache[$id]['text']=$text; }

  function manage_template_received_create ()  { return manage_template_received_update (-1); }
  function manage_template_delete ($id)        { $c=manage_spawn_template ($id); $c->Delete (); }
  ...
  function manage_register_template ($id, $name) {
    global $manage_template_cache;
    $manage_template_cache[$id]=array ('id'=>$id, 'name'=>$name);
  }
  ...
  function manage_template_by_name ($name) {
    ...
    if ($id<0) $id=db_field_value ('templates', 'id', '`name`="'.addslashes ($name).'"');
    if ($id=='') $id=-1;
  ...
  function manage_template_draw_selector_for_script ($name,$active=-1,$cacheable=true) {
    global $manage_templates_arr;
    ...
    for ($i=0; $i<$n; $i++)
      println ('  <option value="'.$manage_templates_arr[$i]->GetID ().'"'.(($manage_templates_arr[$i]->GetID ()==$active)?(' selected'):('')).'>'.$manage_templates_arr[$i]->GetName ().'</option>');
    println ('</select>');
  }

  function manage_template_receive_from_selector ($name) {
    $res=$_POST[$name.'_script_selector'];
    if (!isnumber ($res)) $res=-1;
    return $res;
  }
