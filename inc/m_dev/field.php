<?php if ($_manage_field_included_!='#manage_dield_Included#') {$_manage_field_included_='#manage_dield_Included#'; 
  ...
  class CDataField extends CVirtual {
    ...
    function Create () {
      ...
      $dataset=$this->dataset; $type=$this->datatype; $title=$this->title; $field=$this->field;
      if (trim ($title)=='' || trim ($field)=='' || trim ($type)=='') return false;
      ...
      db_insert ('dataset_assoc', array ('dataset'=>'"'.$dataset.'"', 'datatype'=>"\"$type\"",
        'title'=>"\"$title\"", 'field'=>"\"$field\"", 'settings'=>"\"$s\"", 'order'=>$ord));
    ...
    function Destroy () {
      ...
      if (manage_dataset_refcount ($dataset)>0) return;
    ...
    function UpdateReceived () { return $this->Update (stripslashes ($_POST['title'])); }
    
    function UpdateStoredSettings () {
      db_update ('dataset_assoc', array ('settings'=>'"'.addslashes ($this->SerializeSettings ()).'"'), '`id`='.$this->id);
    }
    
    function ToggleImportancy () {
      $this->settings['important']=!$this->settings['important'];
      if ($this->settings['important']) $this->settings['invisible']=false;
      $this->UpdateStoredSettings ();
    }
    
    function ToggleInvisibility () {
      $this->settings['invisible']=!$this->settings['invisible'];
      if ($this->settings['invisible']) $this->settings['important']=false;
      $this->UpdateStoredSettings ();
    }
    
    function Up   () { $dataset=db_field_value ('dataset_assoc', 'dataset', '`id`='.$this->id); db_move_up ('dataset_assoc', $this->id, '`dataset`='.$dataset); }
    ...
    function DrawEditorForm       ($formname='', $init=true) { $this->datatype->DrawEditorForm ($this->field, $formname, $init); }
    function ReceiveValue         ($formname='') { $this->datatype->ReceiveValue ($this->field, $formname); }
    function BuildQueryValue      ()             { return $this->datatype->BuildQueryValue ($this->value); }
    function BuildCheckImportancy ($formname='') { return $this->datatype->BuildCheckImportancy ($this->field, $formname); }

    function BuildInitScript      ($formname='') { return $this->datatype->BuildInitScript ($this->field, $formname); }

    function DrawContentSettingsForm ($formname='') { $formname.=($formname)?('_'):(''); return $this->datatype->DrawContentSettingsForm ($this->title, $formname.$this->field); }
    function ReceiveContentSettings  ($formname='') { $formname.=($formname)?('_'):(''); return $this->datatype->ReceiveContentSettings  ($this->title, $formname.$this->field); }

    function NewContentSpawned      ($id=-1) { return $this->datatype->NewContentSpawned ($this->field, $id); }
    function PerformContentDeletion ($id=-1) { $this->datatype->PerformContentDeletion ($this->field, $id); }
  ...
  function manage_datafield_create ($dataset, $type, $title, $field, $out) { $out=manage_spawn_datafield (-1, $dataset, $type, $title, $field); return $out->Create ();  }
  function manage_datafield_delete ($id) { $c=manage_spawn_datafield ($id); $c->Destroy ();}
  function manage_datafield_update ($id, $title, $out)    { $out=manage_spawn_datafield ($id); return $out->Update ($title);}
  function manage_datafield_update_received ($id, $out)   { $out=manage_spawn_datafield ($id); $out->UpdateReceived ();}
  function manage_datafield_toggle_importancy ($id) { $c=manage_spawn_datafield ($id); $c->ToggleImportancy (); }
  function manage_datafield_toggle_elem_invisibility ($id) { $c=manage_spawn_datafield ($id); $c->ToggleInvisibility (); }
