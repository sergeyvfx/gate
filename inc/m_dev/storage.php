<?php if ($_manage_storage_included_!='#manage_storage_Included#') {$_manage_storage_included_='#manage_storage_Included#'; 
  ...
  class CStorage extends CVirtual  {
    ...
    function GetFullPath () { return config_get ('site-root').config_get ('document-root').config_get ('storage-root').$this->path; }
    ...
    function GetFullUrl ($v) {
      if ($v=='') return '';
      $res=config_get ('document-root').config_get ('storage-root').$this->path.'/';
      $fn=db_field_value ('storage_volume_'.$this->id, 'file', '`id`='.$v);
      return $res.$fn;
    }
    ...
    function Create () {
      ...
      if (!check_dir ($path)) return false;
    ...
    function UpdateReceived () {
      if ($this->Update (stripslashes ($_POST['name']))) $_POST=array ();
    }
    function Destroy () {
    ...
    function GetFileParams ($id) { if ($this->id<=0) return array (); $r=db_field_value ('storage_volume_'.$this->id, 'params', "`id`=$id"); return unserialize ($r); }
    ...
    function Put ($data, $user_id=-1) {
      ...
      db_update ('storage_volume_'.$this->id, array ('file'=>'"'.addslashes ($file).'"', 'params'=>'"'.addslashes (serialize ($params)).'"'), '`id`='.$id);
      return $id;
    }
    function Accept ($id) { db_update ('storage_volume_'.$this->id, array ('accepted'=>'1'), "`id`=$id"); }
    ...
    function DeleteUnwanted () {
      $q=db_select ('storage_volume_'.$this->id, array ('id'), '`timestamp`<'.(time ()-config_get ('storage-lifetime')).' AND `accepted`=0');
      while ($r=db_row ($q)) $this->Unlink ($r['id']);
    }
  ...
  function manage_storage_refcount ($id)     { return db_field_value ('storage', 'refcount', "`id`=$id"); }
  function manage_storage_refcount_inc ($id) { db_update ('storage', array ('refcount'=>'`refcount`+1'), "`id`=$id"); }
  function manage_storage_refcount_dec ($id) { db_update ('storage', array ('refcount'=>'`refcount`-1'), "`id`=$id"); print db_error ();  }

  function manage_storage_create          ($name, $path, $out=null) { $out=manage_spawn_storage (-1, $name, $path);  return $out->Create (); }
  function manage_storage_create_received ($out=null)               { $out=manage_spawn_storage (-1);  return $out->CreateReceived (); }
  function manage_storage_update          ($id, $out=null)          { $out=manage_spawn_storage ($id); return $out->UpdateReceived (); }

  function manage_storage_delete          ($id)                     { $out=manage_spawn_storage ($id); return $out->Destroy (); }

  function manage_storage_get_list () {
    global $manage_storage_cache;
    $q=db_select ('storage', array ('*'), '', 'ORDER BY `name`');
    while ($r=db_row ($q)) { $manage_storage_cache[$r['id']]=$r;  $arr[]=manage_spawn_storage ($r['id']); }
    return $arr;
  }
  
  function manage_storage_unregister ($id) { global $manage_storage_cache; unset ($manage_storage_cache[$id]); }

  function manage_stroage_exists ($id) { if ($id=='') return false; return db_count ('storage', '`id`='.$id)>0; }
