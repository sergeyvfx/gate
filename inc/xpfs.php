<?php if ($_xpfs_included_!='#xpfs_Included#') {$_xpfs_included_='#xpfs_Included#'; 
  ...
  class XPFS {
    ...
    function nodeDescrFromUnknownArr ($volume, $arr) {
      return array ('vol'=>$volume, 'id'=>$arr['id'], 'pid'=>$arr['pid'], 'access'=>$arr['access'],
                    'attr'=>$arr['attr'], 'mtime'=>$arr['mtime'], 'name'=>$arr['name']);
    }
    ...
    function parsePath ($path) {
      ...
      if ($volume[0]=='/' || $volume=='') $volume=$XPFS_DEFAULT_VOLUME;
    ...
    function renameNode ($node, $new_name) {
      ...
      if (db_count ('xpfs_volume_'.$node['vol'], '`name`="'.addslashes ($new_name).'" AND `id`!='.$node['id'])>0) return false;
      db_update ('xpfs_volume_'.$node['vol'], array ('name'=>'"'.addslashes ($new_name).'"'), '`id`='.$node['id']);
      if (db_affected () > 0) return true;
      return false;
    }
    ...
    function moveNode ($node, $parent_node) {
      if (!$this->isNodeAvaliable ($node)) return false;
      if ($node['vol']!=$parent_node['vol']) return false;
      db_update ('xpfs_volume_'.$node['vol'], array ('pid'=>$parent_node['id']), '`id`='.$node['id']);
    }

    function moveItem ($item, $parent) {
      $_item=$this->parsePath ($item);
      $_parent=$this->parsePath ($parent);
      if ($_item['vol']!=$_parent['vol']) return false;
      return $this->moveVolumeItem ($_item['vol'], $_item['dir'], $_parent['dir']);
    }
    ...
    function lsNode ($node) {
      ...
      if (!$this->isNodeAvaliable ($node)) return $r;

      if ($node['id']>=0) {
        $q=db_select ('xpfs_volume_'.$node['vol'], array ('*'), '`pid`='.$node['id']);
        while ($row=db_row ($q)) {
    ...
    function writeNodeBlock ($node, $content="") {
      ...
      db_update ('xpfs_volume_'.$node['vol'], array ('data'=>'"'.addslashes ($content).'"', 'mtime'=>time ()), '`id`='.$node['id']);
    }
