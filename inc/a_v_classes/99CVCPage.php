<?php if ($_CVCPage_!='#CVCPage_Included#') {$_CVCPage_='#CVCPage_Included#';
  global $main_page;
...
//      add_body_handler ('onunload', 'processForceURL');
    }
    ...
    function SetRSS  ($title, $href) { $this->settings['rss']=array ('title'=>$title, 'href'=>$href); }
    function GetRSS  ()              { return $this->settings['rss']; }

    function AddMeta ($m) {$meta = new CMMeta; $meta->Init ($m); $this->metas[]=$meta;}
    function AddScript ($params,$src='') {$script = new CMScript; $script->Init ($params,$src); $this->scripts[]=$script;}
    function AddScriptFile ($file,$lang='JavaScript') {
      if ($this->appenedScripts[$file]) return;
      $this->appenedScripts[$file]=true;
      $script = new CMScript;
      $script->Init ('language='.$lang.';type=text/'.$lang.';src='.config_get ('document-root').'/scripts/'.$file, '');
      $this->scripts[]=$script;
    }
    ...
    function TPrint ($text) {$this->content->TPrint ($text);} // Print simple text to container
    ...
    function HeadSource () {
      ...
      $rss=$this->GetRSS  ();
      if ($rss['href']!='') {
        $result=swriteln ($result,$this->FromTemplate ('link', array ('rel'=>'alternate', 'type'=>'application/rss+xml',
    ...
    function FillTopMenu () {
      ...
      if (!user_authorized () || user_access_root ()) {
        if (nav_inside ('/admin')>=0)
          $this->topmenu->AppendItem ('Основной раздел', config_get ('document-root').'/', 'main', ''); else
    ...
    function AppendTitle ($v,$last=false) { $this->settings['title'].=(($last)?(' - '):( (($this->title_appended>1)?(' || '):(': ')) )).$v; $this->title_appended++; }
  }
