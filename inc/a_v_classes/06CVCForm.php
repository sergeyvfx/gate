<?php if ($_CVCForm_!='#CVCForm_Included#') {$_CVCForm_='#CVCForm_Included#';
   class CVCForm extends CVCVirtual {
    ...
    function Init ($name='', $settings='') {
      ...
      if ($this->settings['caption']=='')
        $this->settings['caption']='Сохранить'; else
        $this->settings['caption']=htmlspecialchars ($this->settings['caption']);
    }

    function AppendField ($title, $name, $type, $value='', $settings=array ()) { $this->fields[]=array ('title'=>$title, 'name'=>$name, 'type'=>$type, 'value'=>$value, 'settings'=>$settings); }
    function AppendCustomField ($src) { $this->AppendField ('', '', 'Custom', $src); }

    function AppendTextField ($title, $name, $value, $important=false, $settings=array ()) {
      $settings['important']=$important;
      $this->AppendField ($title, $name, 'Text', $value, $settings);
    }

    function AppendPasswordField ($title, $name, $value, $important=false, $settings=array ()) {
      $settings['important']=$important;
      $this->AppendField ($title, $name, 'Password', $value, $settings);
    }

    function AppendEMailField ($title, $name, $value, $important=false, $settings=array ()) {
      $settings['important']=$important;
      $this->AppendField ($title, $name, 'Email', $value, $settings);
    }

    function AppendCheckBoxField ($title, $name, $value, $important=false, $settings=array ()) {
      $settings['important']=$important;
      $this->AppendField ($title, $name, 'CheckBox', $value, $settings);
    }

    function AppendComboBoxField ($title, $name, $value, $items=array (), $important=false, $settings=array ()) {
      $settings['important']=$important;
      $settings['items']=$items;
      $this->AppendField ($title, $name, 'ComboBox', $value, $settings);
    }

    function AppendLabelField ($title, $name, $value, $items=array (), $important=false, $settings=array ()) {
      $settings['important']=$important;
      $this->AppendField ($title, $name, 'Label', $value, $settings);
    }

    function TextFieldHTML ($f) {
      return '    <input type="text" class="txt block"'.(($f['name']!='')?(' name="'.$this->settings['name'].'_'.$f['name'].'" id="'.$this->settings['name'].'_'.$f['name'].'"'):('')).
        ' value="'.htmlspecialchars ($f['value']).'"'.
        '>'."\n";
    }

    function PasswordFieldHTML ($f) {
      return '    <input type="password" class="txt block"'.(($f['name']!='')?(' name="'.$this->settings['name'].'_'.$f['name'].'" id="'.$this->settings['name'].'_'.$f['name'].'"'):('')).
        ' value="'.htmlspecialchars ($f['value']).'"'.
        '>'."\n";
    }
    ...
    function CheckBoxFieldHTML ($f) {
      return '    <input type="checkbox" '.(($f['name']!='')?(' name="'.$this->settings['name'].'_'.$f['name'].'" id="'.$this->settings['name'].'_'.$f['name'].'"'):('')).
    ...
    function ComboBoxFieldHTML ($f) {
      ...
      $res=swriteln ($res, '  <select class="block" name="'.$this->settings['name'].'_'.$f['name'].'">');
      $n=count ($items);
      for ($i=0; $i<$n; $i++) {
        $it=$items[$i];
        $val=$it['value'];
        if (!isset ($val)) $val=$it['id'];
        $res=swriteln ($res, '    <option'.((isset ($val))?(' value="'.$val.'"'):('')).
    ...
    function CUSTOMFieldCheckScript ($f) {
      ...
      return 'qtrim (getElementById ("'.$this->settings['name'].'_'.$field.'").value)!=""';
    }

    function CustomFieldHTML ($f) {
      return tpl_src ($f['value']['src'], array ('value'=>$f['value']['value']));
    }
    ...
    function FieldsHTML () {
      ...
      for ($i=0; $i<$n; $i++) {
        ...
        if ($f['type']!='Custom')
          $res=swriteln ($res, '  <tr'.(($i==$n-1)?(' class="last"'):('')).'><td'.$dummy.'>'.$f['title'].'</td><td>'.$this->FieldHTML ($f).'</td></tr>'); else
          {
    ...
    function TextFieldCheckScript       ($f) { return 'qtrim (getElementById ("'.$this->settings['name'].'_'.$f['name'].'").value)!=""'; }
    function PasswordFieldCheckScript   ($f) { return 'qtrim (getElementById ("'.$this->settings['name'].'_'.$f['name'].'").value)!=""'; }
    function EMailFieldCheckScript      ($f) { return 'check_email (getElementById ("'.$this->settings['name'].'_'.$f['name'].'").value)'; }
    ...
    function CheckScripts () {
      ...
      for ($i=0; $i<$n; $i++) {
        $f=$this->fields[$i];
        if (!$f['settings']['important']) continue;
        $handler=$f['type'].'FieldCheckScript';
        $cond=$this->$handler ($f);
        $src='    if (!('.$cond.')) { alert ("Не указано обязательное поле \"'.$f['title'].'\" или поле содержит ошику."); return false; }';
    ...
    function InnerHTML () {
      $res='';
      $res=swriteln ($res, '<script language="JavaScript" type="text/javascript">');
      $res=swriteln ($res, '  function form_'.$this->settings['name'].'_check (frm) {');
  ...
  function FormPOSTValue ($variable, $formname='') { return $_POST[$formname.'_'.$variable]; }
