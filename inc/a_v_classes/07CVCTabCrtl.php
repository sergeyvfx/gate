<?php if ($_CVCTabCtrl_!='#CVCTabCtrl_Included#') {$_CVCTabCtrl_='#CVCTabCtrl_Included#';
  class CVCTabCtrl extends CVCVirtual {
    ...
    function StartNewPage ($params) {
      if ($this->pageStarted) $this->FinishPage ();
      $p=unserialize_params ($params);
      if ($p['active']) $this->active=count ($this->pages);
      $n=count ($this->pageParams); $arr=array ();
      for ($i=0; $i<$n; $i++) $arr[$this->pageParams[$i]]=$p[$this->pageParams[$i]];
      $this->pages[]=$arr;
    ...
    function SetActive ($n) {
      if ($n<0) $n=0;
      if ($n>=count ($this->pages)) $n=count ($this->pages)-1;
      $this->active=$n;
    }

    function OuterHTML () {
      $res=''; $n=count ($this->pages);
      $p="count=$n;";
      for ($i=0; $i<$n; $i++) {
        $p.='tab'.$i.'='.prepare_arg ($this->pages[$i]['title']).';';
      }

      if ($this->settings['name']=='') $suff='tabctrl'; else $suff=$this->settings['name'];
      
      if (isset ($GLOBALS[$suff]) && $GLOBALS[$suff]!='')
        $active=$GLOBALS[$suff]; else $active=0;
      if ($active<0) $active=0;
      if ($active>=count ($this->pages)) $active=count ($this->pages)-1;

      $p.=';active='.$active;

      $url=$this->settings['url'];
      if ($url=='') $url=content_url_get_full ();
