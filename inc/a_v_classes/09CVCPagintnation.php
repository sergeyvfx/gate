<?php if ($_CVCPagintation_!='#CVCPagintation_Included#') {$_CVCPagintation_='#CVCPagintation_Included#';
   class CVCPagintation extends CVCVirtual {
    ...
    function Init ($name='', $settings='') {
      ...
      if ($this->settings['urlprefix']=='') $this->settings['urlprefix']=content_url_get_full ();
      $s=$this->settings['urlprefix'];
      if (preg_match ('/\?/', $s)) {
        if (!preg_match ('/\?$/', $s)) $s.='&';
      } else $s.='?';
      $this->settings['urlprefix']=$s;
      if ($this->settings['pageid']=='') $this->settings['pageid']='page';
      $this->topStencil=$this->bottomStencil=$this->defaultStencil;
      if ($this->settings['perpage']=='') $this->settings['perpage']=1;
    }

    function CopyStencil      ($s) { return array ('outer'=>$s['outer'], 'item'=>$s['item']); }
    function SetTopStencil    ($s) { $this->topStencil=$this->CopyStencil ($s); }
    function SetBottomStencil ($s) { $this->bottomStencil=$this->CopyStencil ($s); }

    function AppendPage ($src) { $this->pages[]=array ('src'=>$src, 'count'=>$this->settings['perpage']); }
    function AppendItem ($src) {
      $uk=count ($this->pages)-1;
      if ($uk<0 || $this->pages[$uk]['count']>$this->settings['perpage'])
        $this->pages[]=array ('src'=>$src, 'count'=>1); else {
    ...
    function GetActivePage () {
      ...
      if ($active>=count ($this->pages)) return count ($this->pages)-1;
      return $active;
    }

    function GetPagintation ($stencil) {
      if ($this->settings['skiponcepage'] && count ($this->pages)<=1) return '';
      $items='';
      ...
      for ($i=0; $i<count ($this->pages); $i++) {
        if ($i!=$active) $st=$itStencil; else $st=$itActiveStencil;
        $itemTmp=preg_replace ('/\$\(title\)/', $i+1, $st);
        $itemTmp=preg_replace ('/\$\(href\)/', $urlprefix.$pageid.'='.$i, $itemTmp);
        $items.=$itemTmp;
      }
    ...
    function SetCurrentPage ($n) { $this->settings['active']=$n; }

    function GetTopPagintation    () { return $this->GetPagintation ($this->topStencil); }
    function GetBottomPagintation () { return $this->GetPagintation ($this->bottomStencil); }
