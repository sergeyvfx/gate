<?php
  ...
  for ($i=0; $i<$cfg['keystring_length']; $i++) {
    ...
    $y=mt_rand(-$cfg['fluctuation_amplitude'], $cfg['fluctuation_amplitude'])+($h-$fontfile_height)/2+2;
    ...
    if ($cfg['no_spaces']) {
      ...
      if ($i>0) {
        ...
        for ($sy=7; $sy<$fontfile_height-20; $sy+=1) {
         for ($sx=$m['start']-1; $sx<$m['end']; $sx+=1) {
            ...
            if ($opacity<127) {
              ...
              $py=$sy+$y;
              if ($py>$h) break;
              for ($px=min ($left, $w-1); $px>$left-12 && $px>=0;$px-=1) {
        ...
        if ($shift==1000) $shift=mt_rand (4,6);
      }
    } else {
      $shift = 1;
    }

    imagecopy ($text_img, $font, $x-$shift, $y, $m['start'], 1, $m['end']-$m['start'], $fontfile_height);
  ...
  imagefilledrectangle ($new_text_img, 0, 0, $w*2, $h*2, imagecolorallocate ($new_text_img, $bg_rgb[0], $bg_rgb[1], $bg_rgb[2]));
  ...
  for ($x=0; $x<$w; $x++) {
    for($y=0; $y<$h; $y++) {
      $sx=$x+(sin($x*$rand1+$rand5)+sin($y*$rand3+$rand6))*$rand9-$w/2+$center+1;
      ...
      imagesetpixel ($new_text_img, $x, $y, imagecolorallocate ($text_img, $newred, $newgreen, $newblue));
    }
  }
  ...
  $color_border = imagecolorallocate ($img, $cfg['colors']['border'][0], $cfg['colors']['border'][1], $cfg['colors']['border'][2]);
  ...
  if ($cfg['signature']['text']!='') {
    $sig_fg  = imagecolorallocate ($img, $cfg['signature']['foreground'][0], $cfg['signature']['foreground'][1], $cfg['signature']['foreground'][2]);
    $sig_bg  = imagecolorallocate ($img, $cfg['signature']['background'][0], $cfg['signature']['background'][1], $cfg['signature']['background'][2]);
    imagefilledrectangle ($img, 0, $h-12, $w-1, $h-12, $color_border);
    imagefilledrectangle ($img, 1, $h-11, $w-2, $h-2, $sig_bg);
    imagestring ($img, 2, $w/2-ImageFontWidth(2)*mb_strlen($cfg['signature']['text'])/2, $h-ImageFontHeight(2), $cfg['signature']['text'], $sig_fg);
    $delta_h=-ImageFontHeight(2);
  }

  $dw=$fx-$sx;
  $dh=$fy-$sy;
  imagecopy ($img, $new_text_img, ($w-$dw)/2, ($h-$dh+$delta_h)/2, $sx, $sy, $dw, $dh);
