<?php if ($_editor_included_!='#editor_Included#') {$_editor_included_='#editor_Included#'; 
  $editor_menus=array ();
  $editor_functions=array ();
  $editor_usage=array ();
  function editor_initialize ($name='default') {
    global $editor_menus, $function;
    if (isset ($editor_menus[$name])) return;
    $editor_menus[$name]=new CVCMenu ();
    $editor_menus[$name]->Init ('editor_menu', 'type=hor;colorized=true;sublevel=0;border=thin;');
  ...
  function editor_add_function ($title, $function, $name='default', $args='') {
    ...
    if ($editor_usage [$title][$function][$name]) return;
  ...
  function editor_set_function ($function, $name='default') { global $editor_menus; $editor_menus[$name]->SetActive ($function);  }
  ...
  function editor_get_valid_function ($name='default') { global $function, $editor_functions; if (!isset ($editor_functions[$name][$function])) $function=0; }

  function editor_draw_menu ($name='default') {
    global $function, $editor_functions, $editor_menus;
    if (!isset ($editor_menus[$name])) return;
    if (!isset ($editor_functions[$name][$function]['function'])) $editor_menus[$name]->SetActiveByIndex (0);
