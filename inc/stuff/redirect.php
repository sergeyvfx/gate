<?php if ($_redirect_included_!='#redirect_Included#') {$_redirect_included_='#redirect_Included#'; 
  $redirecting=false;
  ...
  function get_redirection ($encode=true,$overwrite=false) {
    ...
    $result='';
    foreach ($_GET as $k=>$v) {
      if ($redirector_skip_vars[$k]['##EMPTY##'] || $redirector_skip_vars[$k][$v]) continue;
      if (strtolower($k)=='redirect' && !$overwrite) {
        if ($encode)  return urlencode ($v); else return $v;
      } else {
  ...
  function redirect ($url='', $skipvars=array ()) {
    global $redirect;
    foreach ($skipvars as $k=>$v) redirector_add_skipvar ($k, $v);
    if ($url=='SELF') $url=get_redirection (false);
    if ($url!='') $redirect=$url;
    if ($redirect=='') $redirect=config_get ('document-root');
  ...
  function redirector_add_skipvar ($name, $val='') {
    if ($val=='') $val='##EMPTY##';
    global $redirector_skip_vars; $redirector_skip_vars[$name][$val]=true;
  }
  ...
  function redirector_remove_skipvar ($name, $val='') {
    if ($val=='') $val='##EMPTY##';
    global $redirector_skip_vars; unset ($redirector_skip_vars[$name][$val]);
  }
