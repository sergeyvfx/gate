...
<?php
  if ($action=='delete') {
    ...
    $value='';
?>
    <script language="JavaScript">window.parent.CDCFile_OnFileUpload ('<?=$field;?>', '<?=$formname;?>', '', '');</script>
<?php
  } else
  if (isset ($_FILES['uploading'])) {
    ...
    ?>
    <script language="JavaScript">window.parent.CDCFile_OnFileUpload ('<?=$field;?>', '<?=$formname;?>', '<?=$full;?>', '<?=$fn;?>', '<?=$p['mime'];?>', '<?=$p['size'];?>');</script>
<?php    } else { ?>
