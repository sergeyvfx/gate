<?php if ($_files_included_!='#files_Included#') {$_files_included_='#files_Included#'; 
  ...
  function file_writeblock ($fn, $data='') { create_file   ($fn, $data); }
  function create_dir ($d) { mkdir ($d); chmod ($d, 0775); }
  ...
  function file_store_encrypted ($data, $access=0, $blocked=0) {
    ...
    for (;;) {
      ...
      if (db_count ('files', "`name`=\"$t\"")==0) break;
    }
