<?php if ($_sec_included_!='#sec_Included#') {$_sec_included_='#sec_Included#';
  ...
  class CSecurityInformation extends CVirtual {
    ...
    function SetDefaultData () { $this->data=array ('inherit'=>true); }
    ...
    function ReceiveData () {
      ...
      if ($_POST[$this->name.'_inherit'] && $this->canInherit) {
        ...
        foreach ($limits as $l=>$v) {
          if (preg_match ('/\{\{\{\{GROUP[0-9]+\}\}\}\}/', $l) || !isset ($this->security_limits[$l])) continue;
          $limit=db_unpack ($v);
          $order=$limit['order'];
          $res[$l]['order']=$order;
          if (!isset ($security_orders[$order])) continue;
          $acts=db_unpack ($limit['acts']); $i=0;
          while (isset ($acts[$i])) {
    ...
    function EditForm () {
      ...
      if (!isset ($securoty_builtin_included)) {
        $CORE->AddScript ( 'language=JavaScript;', tpl ('back/security/script', array ('limits'=>$this->security_limits,
          'orders'=>$security_orders, 'actions'=>$security_actions), false));
        $CORE->AddStyle ('security');
        $securoty_builtin_included=true;
      }
      print tpl ('back/security/edit_form', array ('name'=>$this->name, 'data'=>$this->data, 'canInherit'=>$this->canInherit, 'limits'=>$this->security_limits));
    }
    ...
    function SetCanInherit ($v) { $this->canInherit=$v; $this->data['inherit']=$v; }

    function RefreshCachedData ($user_id=-1) {
      if (isset ($this->precompiled[$user_id]['user_group'])) return;
      $user_group=array ();
      if ($user_id>0) {
  ...
  function security_initialize () {
    if (config_get ('check-database')) {
      if (!db_table_exists ('user')) {
        ...
        db_insert ('user', array ('name'=>'"root"', 'login'=>'"root"', 'password'=>'MD5("root#RANDOM_SEED#assword")',
          'access'=>'7', 'authorized'=>'1', 'settings'=>'""', 'email'=>'"postmaster@localhost"'));
      }
  ...
  function security_access_title ($a) {
    $arr=security_groups ();
    for ($i=0; $i<count ($arr); $i++)
      if ($arr[$i]['access']==$a) return $arr[$i]['title'];
    return '<Неизвестно>';
  }
}
?>
