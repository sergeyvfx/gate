<?php if ($_sec_group_included_!='#sec_group_Included#') {$_sec_group_included_='#sec_group_Included#';
  function group_create ($name, $default=false) {
    ...
    if (db_count ('group', '`name`="'.$name.'"')>0) {
      ...
      db_insert ('group', array ('name'=>'"'.addslashes ($name).'"', 'default'=>($default)?(1):(0)));
      return true;
    }
  }
  ...
  function group_update ($id) {
    ...
    if (db_count ('group', '(`name`="'.$name.'")  AND (`id`<>'.$id.')')>0)
      add_info ('Группа с таким именем уже существует.'); else
      db_update ('group', array ('name'=>'"'.$name.'"', 'default'=>(manage_setting_get_received ('default_group'))?(1):(0)), "`id`=$id");
  }
  function group_list () { return arr_from_query ('SELECT * FROM `group` ORDER BY `name`'); }
  function group_users_inside ($id) { return db_field_value ('group', 'refcount', '`id`='.$id); }
  function group_default_list () { return arr_from_query ('SELECT * FROM `group` WHERE `default`=1 ORDER BY `name`');}
}