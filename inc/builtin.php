<?php if ($_builtin_included_!='#builtin_Included#') {$_builtin_included_='#builtin_Included#';
  ...
  function mtime   ()      { list ($sec, $msec)=explode (' ', microtime ()); return $sec+$msec; }
  function inarr ($arr, $item) { for ($i=0; $i<count ($arr); $i++) if ($arr[$i]==$item) return true; return false; }
  ...
  function unserialize_params ($s) {
    ...
    while ($i<$n) {
      $c=$s[$i];
      if ($c=='=') { $variable=$token; $token=''; } else
      if ($c==';') {
        if ($variable!='') $result[$variable]=$token;
        $variable=$token='';
      } else
      if ($c=='\\') {
        $c=$s[++$i];
  ...
  function arr_from_ret_query ($q, $field='') { $arr=array (); while ($r=db_row ($q)) if ($field=='') $arr[]=$r; else $arr[]=$r[$field]; return $arr; }
  function arr_from_query ($query, $field='') { $q=db_query ($query); return arr_from_ret_query ($q, $field); }
  function isnumber ($a, $signed=false) { if ($signed) return preg_match ('/^([\+\-])?[0-9]+$/', $a); return preg_match ('/^[0-9]+$/', $a); }
  ...
  function check_dir ($s)     { if (strlen ($s)==0) return false; return preg_match ('/(\/[A-Z0-9a-z]+)+\/?/', $s); }
  ...
  function _get_browser   () { $browser=get_browser_cacheable (); return $browser['browser']; }
  function browser_engine () { $browser=get_browser_cacheable (); return $browser['engine']; }
  
  function eval_code ($code) { ob_start (); eval ('?>'.$code); $r=ob_get_contents (); ob_end_clean (); return $r; }

  function get_real_ip () {
    global $_SERVER;
    if (!empty ($_SERVER['HTTP_X_FORWARDER_FOR']))
      return $_SERVER['HTTP_X_FORWARDED_FOR']; else
  ...
  function format_ltime ($self, $use_short_days=true) { return format_ldate ($self, $use_short_days).', '.date ('H:i:s', $self); }
  ...
  function add_info ($txt) { global $CORE; if ($CORE==nil) return;  $CORE->PAGE->AddInfo ($txt); }
  ...
  function nav_inside ($url) {
    ...
    $url=preg_replace ('/\/+/', '/', $url);
    $dir=get_cur_dir ();
    if (!preg_match ('/^'.prepare_pattern ($url).'/', $dir)) return -1;
    if ($url!='/')
      $difference=preg_replace ('/^'.prepare_pattern ($url).'/', '', $dir); else
      $difference=$url;
    $difference=preg_replace ('/\/+$/', '', $difference);
    if ($difference=='') return 0; // full entry
    if ($difference[0]!='/') return -1;
    $entries=0;
    for ($i=0; $i<strlen ($difference); $i++)
      if ($difference[$i]=='/') $entries++;
    return $entries;
  }

  function check_locked () {
    global $login, $passwd;
    if (!opt_get ('site_lock')) return false;
    if ($login!='')
      user_authorize ($login, $passwd);
    if (user_access_root ()) return false;
  ...
  function FullLocalTime ($t='', $add_timezone=true) { if ($t=='') $t=time (); return date ('D, d M Y h:i:s '.((!$add_timezone)?(''):('+'.config_get ('time-zone'))), $t); }
