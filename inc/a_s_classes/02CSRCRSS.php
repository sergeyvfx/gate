<?php if ($_CSCRSS_included_!='#CSCRSS_Included#') {$_CSCRSS_included_='#CSCRSS_Included#';
  class CSCRSS extends CSCVirtual {
    ...
    function CSCRSS () {
      $this->SetServiceName ('RSS Feed'); $this->SetClassName ('CSCRSS');
      $this->filename=config_get ('site-root').config_get ('document-root').'/rss.php';
    }
    ...
    function CanCreate () {
      if (db_count ('service', '`sclass`="CSCRSS"')==0) return true;
      add_info ('Может существовать лишь один RSS серфис.');
      return false;
    }

    function DrawSettingsForm ($formnane='') {
      print ('Заголовок:<input type="text" class="txt block" name="'.$formname.'_RSSTitle"><div id="hr"></div>'."\n");
      print ('Описание:<input type="text" class="txt block" name="'.$formname.'_RSSDescription"><div id="hr"></div>'."\n");
      print ('URL:<input type="text" class="txt block" name="'.$formname.'_RSSURL">'."\n");
    }
...
');
    ...
    function GetRSSData () {
      ...
      if (is_array ($this->settings['contents']))
        foreach ($this->settings['contents'] as $cid=>$dummy) {
          ...
          if ($c->GetID ()<=0) { unset ($this->settings['contents'][$cid]); continue; }
    ...
    function Editor_RSSManage () {
      ...
      print '<form action="'.$full.'&act=save" method="POST">'."\n";
      print 'Заголовок: <input type="text" class="txt block" value="'.htmlspecialchars ($this->settings['title']).'" name="title"><div id="hr"></div>'."\n";
      print 'Описание: <input type="text" class="txt block" value="'.htmlspecialchars ($this->settings['description']).'" name="description"><div id="hr"></div>'."\n";
      print 'URL: <input type="text" class="txt block" value="'.htmlspecialchars ($this->settings['url']).'" name="url">'."\n";
    ...
    function GetContentsByPid ($pid, $list) {
      $arr=array ();
      for ($i=0; $i<count ($list); $i++) { if ($list[$i]['pid']==$pid) $arr[]=$list[$i]; }
      return $arr;
    }
    ...
    function PrintContents ($pid, $list, $pIndexes=array (), $path='', $afterInfo=false) {
      ...
      $n=count ($arr);
      $cntPrefix='';
      for ($i=0; $i<count ($pIndexes); $i++) $cntPrefix.=$pIndexes[$i].'.';
      ...
      for ($i=0; $i<$n; $i++) {
        ...
        $checked=false;
        if ($this->settings['contents'][$r['id']]) $checked=true;
        print (
  ...
  '</tr></table></div>');
    ...
    function Editor_ContentManage () {
      ...
      print '<form action="'.content_url_get_full ().'&act=save" method="post">';
