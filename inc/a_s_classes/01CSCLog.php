<?php if ($_CSCLog_included_!='#CSCLog_Included#') {$_CSCLog_included_='#CSCLog_Included#';
  class CSCLog extends CSCVirtual {
    ...
    function CSCLog () { $this->SetServiceName ('Бортовой журнал'); $this->SetClassName ('CSCLog'); }
    ...
    function CanCreate () {
      if (db_count ('service', '`sclass`="CSCLog"')==0) return true;
    ...
    function AppendEntry ($module, $status, $message) {
      ...
      db_insert ('log', array ('timestamp'=>time (), 'module'=>"$module", 'status'=>"$status",
        'message'=>"$message"));
      return true;
    }
    ...
    function Editor_LogView () {
      ...
      if ($count!=512 && $count!=1024 && $count!=-1) $count=256;
      $clause='';
...
<?       if (db_affected ()>0) { ?>        
        <div class="scroll" style="height: 320px; margin-top: 4px;" id="log">
<?php        
        while ($r=db_row ($q)) {
          $class="msg";
  ...
  ?>
         <div class="<?=$class;?>"><table><tr><td class="time"><?=format_ltime ($r['timestamp']);?></td><td class="module"><?=$r['module'];?></td><td class="status"><?=$status;?></td><td class="msg"><?=$r['message'];?></td></tr></table></div>
  <?
        }
        print '</div>';
      } else print '<center><i>Файл журнала пуст</i></center>';
      formc ();
    }
    function Editor_LogManage () {
      ...
      $CORE->AddStyle ('log');
      if (isset ($active)) {
        if ($active=='0') $this->settings['active']=false; else $this->settings['active']=true;
        $this->UpdateSettings ();
      }
      if ($act=='free') $this->FreeLogs ();
