<?php
  ...
  global $id;
  if ($id==1) include 'editroot.php'; else {
...
<? } ?>
  ...
  function check (frm) {
    ...
    if (!check_folder (path)) { alert ('Название виртуальной папки является некоррекнтым.'); return false; }
  ...
  function check_path () {
    ...
    if (!check_folder (path)) { show_msg ('path_check_res', 'err', 'Название виртуальной папки является некорректным.'); return false; }
