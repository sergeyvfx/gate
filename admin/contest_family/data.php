<?php
  ...
  $datacontrol_menu->SetActive ('contest_family');

  if ($action == 'create') {
      contestFamily_create_received();
  }
  ...
  if ($action == 'edit') {
    ...
    if ($action == 'save') {
      manage_contestFamily_update_received ($id);
    } else if ($action=='delete') {
      manage_contestFamily_delete ($id);
    }

    $list = manage_contestFamily_get_list ();
    if (count ($list)) {
      include 'list.php';
    }
