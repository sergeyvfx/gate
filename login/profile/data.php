<?php if ($PHP_SELF!='') {print 'HACKERS?'; die;} 
  if (!user_authorized ()) header ('Location: ..?redirect='.get_redirection ());
  ...
  if ($action=='save') {
    ...
    if ($u['email']!='' && !check_email ($email)) add_info ('Указанный E-Mail не выглядит корректным'); else
    if (user_registered_with_email ($email, user_id ())) add_info ('Такой E-Mail уже используется.'); else
