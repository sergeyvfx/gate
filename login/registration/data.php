...
function check_passwd () {
  ...
  if (passwd=='' && confirm=='') { widget.innerHTML=''; return;  }
...
function check () {
  ...
  if (!isalphanum (login) || qtrim (login)=='') { alert ('Логин создаваемого пользователя может состоять лишь из букв латинского алфавита и цифр и не может быть пустым.'); return false; }
  if (qtrim (login).length><?=opt_get ('max_user_login_len');?>) {alert ('Логин создаваемого пользователя может содержать не более <?=opt_get ('max_user_login_len');?> символов.'); return false;}
  if (qtrim (name)=='') { alert ('Имя создаваемого пользователя не может быть пустым.'); return false; }
  if (qtrim (name).length><?=opt_get ('max_user_name_len');?>) {alert ('Имя создаваемого пользователя может содержать не более <?=opt_get ('max_user_name_len');?> символов.'); return false;}
  if (!check_email (getElementById ('email').value)) {alert ('Адрес электронной не является корректным.'); return false;}
  if (passwd!=passwd_confirm) { alert ('Ошибка подтверждения пароля.'); return false; }
  if (passwd.length><?=opt_get ('max_user_passwd_len');?>) {alert ('Пароль создаваемого пользователя может содержать не более <?=opt_get ('max_user_passwd_len');?> символов.'); return false;}
  if (!agree.checked) { alert ('Вы не согласны с правилами этого ресурса, так как же мы Вас здесь зарегестрируем?'); return false; }
  return true;
}
...
function check_login () {
  var login=getElementById ('login').value;
  if (!isalphanum (login) || qtrim (login)=='') {show_msg ('login_check_res', 'err', 'Логин создаваемого пользователя может состоять лишь из букв латинского алфавита и цифр и не может быть пустым.'); return false;}
  if (qtrim (login).length><?=opt_get ('max_user_login_len');?>) {show_msg ('login_check_res', 'err', 'Логин пользователя может содержать не более 14 символов.'); return false;}
    ipc_send_request ('/', 'ipc=check_login&login='+login, update_login_check);
  }

function check_frm_email () {
  var email=getElementById ('email').value;
    if (!check_email (email)) {show_msg ('email_check_res', 'err', 'Указанный почтовый ящик не выглядит корректным. Его использование невозможно.'); return false;}
...
<?php
  ...
  function register () {
    ...
    if ($_SESSION['CAPTCHA_Keystring']=='' || strtolower ($keystring)!=$_SESSION['CAPTCHA_Keystring']) { add_info ('Вы не прошли тест Тьюринга на подтверждение того, что вы не бот.'); return false; }
    if ($email==config_get ('null-email')) { add_info ('Недопустимый адрес электронной почты.'); return false; }
    if (!$agree) { add_info ('Вы не согласны с правилами этого ресурса, так как же мы Вас здесь зарегестрируем?'); return false; }
