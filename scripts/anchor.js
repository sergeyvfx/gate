...
var anchor_Deletion=null, anchor_Src=null, anchor_Dst=null, anchor_Callback=null;
...
function anchor_UpdateHover (x, y, id) {
  ...
  for (var i=0; i<anchor_Registered.length; i++) { 
    ...
    if (anchor.id!=id && core_PointInRectangle (x+scroll.scrollX, y+scroll.scrollY, anchor.rec)) {
...
function anchor_UpdateDraggingPosition (event) {
  ...
  if (dragging.rollback) return;
  var node=dragging.node;
  var scroll=core_GetPageScroll ();
  var startScroll=anchor_GetStartScroll ();
  anchor_UpdateHover (event.clientX, event.clientY, dragging.id);
  var dx=scroll.scrollX-startScroll.scrollX, dy=scroll.scrollY-startScroll.scrollY;
  var sCoords=anchor_GetStartMouseCoords ();
  var cdx=event.clientX-sCoords.x, cdy=event.clientY-sCoords.y;
...
function anchor_CallCallback (source, destination) {
  ...
  for (var i=0; i<anchor_Registered.length; i++) {
    ...
    if (anchor.id==source) {
      ...
      anchor_Src=source; anchor_Dst=destination;
