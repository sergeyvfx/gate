...
function UnregisterDialog (id) {
  ...
  if (dialogs_locked) return;

  for (var i=0; i<opened_dialogs_count; i++)
    if (opened_dialogs[i].id==id) { found=1; break; }
  if (found)
    {
...
function ShowDialog (w, h, html, onok, oncancel, active) {
  var t=(screen.height-h)/2, l=(screen.width-w)/2;

  var params='scrollbars=0,fullscreen=0,status=0,toolbar=0,width='+w+'px,height='+h+'px,resizable=0,top='+t+'px,left='+l+'px';
  var wnd=open ('', '', params);
  ...
  s_sheet.setAttribute ('rel','stylesheet'); s_sheet.setAttribute ('type','text/css');
