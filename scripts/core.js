...
function show_msg (id, type, text) {
  ...
  if (type=='ok') node.className='msg_ok'; else
  if (type=='err') node.className='msg_err';
...
function ipc_send_request (addr, post, callback) {
  if (engine=='GECKO' || engine=='OPERA')
    var http_request=new XMLHttpRequest (); else
  if (engine=='DONKEY')
    var http_request=new ActiveXObject('Microsoft.XMLHTTP'); else {
