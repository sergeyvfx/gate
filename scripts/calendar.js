...
var Moths=['Января','Февраля','Марта','Апреля','Мая','Июня','Июля','Августа','Сентября','Октября','Ноября','Декабря'];
function get_cdays_of_month (month, year) {
  if ((year%4==0 && year%100!=0) || (year%400==0)) return VCDMonth[month-1];
  return CDMonth[month-1];
}
function get_first_day_of_month (month, year) {
  var cMon=1, cYear=1972, cDay=6;
  if (year<1972) {alert ('Слишком старый год!'); return 0;}
  while (cYear!=year || (cYear==year && cMon<month)) {
    cDay=(cDay+get_cdays_of_month (cMon, cYear))%7;
    if (cDay==0) cDay=7;
    cMon++;
    if (cMon==13) {cMon=1; cYear++;}
  }
  return cDay;
}
...
function calendar_updateDays (name) {
  ...
  var pm=m-1, py=y;
  if (pm<1) { pm=1; py--; }
  var n=get_cdays_of_month (m, y), f=get_first_day_of_month (m, y);
  ...
  for (var i=0; i<5; i++)
    for (var j=0; j<7; j++) {
      ...
      if (act==0 || act==2) {
        ...
        node.tag=day;
        if (day==d) node.className='c';
      }
      if (j>=5) node.className+=' h';
    }
}
...
function calendar_setCurrentDate (name, date) { var s=date.y+'-'+date.m+'-'+date.d; getElementById (name).value=s; calendar_updateTitle (name); }
...
function calendar_updateYear (name, year)   { var cDate=calendar_getCurrentDate (name); cDate.y=year; calendar_setCurrentDate (name, cDate); }

function calendar_updateMonth (name, month) { var cDate=calendar_getCurrentDate (name); cDate.m=month; calendar_setCurrentDate (name, cDate); }
