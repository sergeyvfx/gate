...
import tablechecker.frames.logic.InfMech;
...
public class Logic {
  ...
  private boolean bindEnumSlot(ISlot slot, HashMap<String, String> data,
          DefaultMutableTreeNode root, ArrayList<String> path) {
    ...
    if (val == null) {
      return slotVal == null || slotVal.equals("Неважно") || slotVal.equals(
              "Неизвестно");
    }
    ...
    root.add(new DefaultMutableTreeNode(
            "Экземпляр: " + val + ", прототип: " + slotVal));
  ...
  private boolean bindSubframeSlot(ISlot slot, HashMap<String, String> data,
          DefaultMutableTreeNode root, ArrayList<String> path) {
    ...
    DefaultMutableTreeNode node = new DefaultMutableTreeNode(
            "Слот " + slot.getName() + " - субфрейм(" + frame.getName() + "), переход к привязке субфрейма");
    root.add(node);
  ...
  private boolean bindProductionalSlot(ISlot slot, HashMap<String, String> data,
          DefaultMutableTreeNode root, ArrayList<String> path) {
  ...
  private boolean bindSlot(ISlot slot, HashMap<String, String> data,
          DefaultMutableTreeNode root, ArrayList<String> path) {
    boolean result = true;
    ...
    DefaultMutableTreeNode node = new DefaultMutableTreeNode("Попытка привязать слот " + slot.
            getName());
    root.add(node);
    ...
    if (result) {
      node.add(new DefaultMutableTreeNode(
              "Слот " + slot.getName() + " был успешно привязан"));
    } else {
      node.add(new DefaultMutableTreeNode(
              "Слот " + slot.getName() + " не был привязан"));
    }
  ...
  private void bindSubSituations(HashMap<String, String> data,
    ...
    for (;;) {
      ...
      HashMap<String, String> currentData = getCurrentData(data, agent,
              recipient);
      ...
      DefaultMutableTreeNode node = new DefaultMutableTreeNode("Реципиент: ТС " + Integer.
              toString(recipient + 1));
      root.add(node);
  ...
  private boolean bindFrame(Frame frame, HashMap<String, String> data,
          DefaultMutableTreeNode root, ArrayList<String> path) {
    ArrayList<ISlot> slots = frame.getSlots();
    String spath = getPath(path);
    boolean ok = false;

    DefaultMutableTreeNode node = new DefaultMutableTreeNode("Попытка привязать фрейм " + frame.
            getName());
    root.add(node);
    ...
    if (!ok) {
      ...
      if (slot != null && (slot.getValue().getValue().equals("Нет") || slot.
              getValue().getValue().equals("Неважно"))) {
        node.add(new DefaultMutableTreeNode(
                "Фрейм " + frame.getName() + " был успешно привязан"));
        return true;
      }
          ...
          case Slot.SUBFRAME:
            ...
            if (dframe != null) {
              ...
              if (pSlot == null || pSlot.getValue() == null
                      || (!pSlot.getValue().getValue().equals("Нет") && !pSlot.
                      getValue().getValue().equals("Неважно"))) {
                couldIgnore = false;
              }
            }
      ...
      node.add(
              ...
              "Фрейм " + frame.getName() + " не был привязан (отсутствует в экземпляре)"));
      ...
      if (slot != null && slot.getValue().getValue().equals("Нет")) {
        node.add(new DefaultMutableTreeNode(
                "Фрейм " + frame.getName() + " не был привязан"));
        return false;
      }
    }
    ...
    for (ISlot slot : slots) {
      ...
      if (slot.getName().equals("Действие") || slot.getName().equals("X") || slot.
              getName().equals("Y")
  ...
  private HashMap<String, String> getCurrentData(HashMap<String, String> data,
          int agentNumber, int recipientNumber) {
  ...
  private int[] buildPoint(HashMap<String, String> data, int agent) {
    ...
    if (data.get("Агент.X") != null && agent == -1) {
      return new int[]{new Integer(data.get("Агент.X")), new Integer(data.get(
                "Агент.Y"))};
    } else if (data.get(a + ".X") != null) {
      return new int[]{new Integer(data.get(a + ".X")), new Integer(data.get(
                a + ".Y"))};
    }
  ...
  public int[] run(HashMap<String, String> data, DefaultMutableTreeNode root) {
    ...
    for (;;) {
      ...
      agent = curAgent;
      DefaultMutableTreeNode node = new DefaultMutableTreeNode("Вывод действия для ТС " + Integer.
              toString(curAgent + 1));
      root.add(node);

      if (bindFrame(rootFrame, data, node, path)) {
        if (data.get("Действие") != null && data.get("Действие").equals(
                "Продолжать движение")) {
